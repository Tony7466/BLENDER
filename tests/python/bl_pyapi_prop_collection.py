...
class BaseTestForeachGetSet(unittest.TestCase):
    ...
    # Native byteorder would be specified with '=', but NumPy uses implicit native byteorder and native size (no buffer
    # prefix) instead in that case.
    ...
    def sequence_generator(collection, prop_rna, is_set, dtype_modifier=None):
        ...
        if is_set:
            # When setting values, each new sequence must have different values to the previously set sequence, so that
            # it is possible to check that setting the values worked.
            ...
            # properties, so that it is possible to check that getting the values worked.
            if prop_rna.type == 'ENUM':
    ...
    def do_get_subtest(self, collection, prop_rna, seq):
        ...
        Returns whether the tested sequence was accessed as a buffer.
        """
        prop_name = prop_rna.identifier
        ...
        if isinstance(seq, np.ndarray):
            # View the sequence as a subtype that allows checking whether the sequence was accessed as a sequence or as
            # a buffer.
            ...
            try:
                ...
                # While it is possible some other error occurred, it is most likely that an attempt was made to write to
                # the memoryview as a sequence.
                return False
            else:
    ...
    def get_sequence_description(seq):
        ...
        if isinstance(seq, (list, tuple)):
            ...
            # When `long long` and `unsigned long long` are larger than 64-bit, NumPy cannot expose explicit-byteorder
            # `long long` ("q") and `unsigned long long` ("Q") arrays as buffers either.
            # Getting `.data` will fail in these cases.
            try:
                buffer_format = seq.data.format
            except ValueError:
                assert element_type.char == "g" or (element_type.char in "qQ" and element_type.itemsize > 8)
                buffer_format = None
    ...
    def do_getset_subtest(self, collection, prop_rna, is_set, *, sequence_type, assert_buffer_usage=False, is_buffers,
        ...
        with self.subTest(prop_name=prop_rna.identifier, sequence_type=sequence_type):
            ...
            if assert_buffer_usage:
                # Due to property raw types not being exposed to Blender's Python API, it is not possible to know which
                # kind of buffer will be compatible with a property. By testing each kind of buffer that could be
                # compatible, the only expectation is that at least one will be considered compatible.
    ...
    def check_foreach_getset(self, collection, prop_rna, is_set):
        ...
        # Buffers with format prefixes are currently considered to be incompatible with the property (falling back to
        # accessing as a sequence), however, byteorder is irrelevant for some buffers, such as `np.byte`
        # (`signed char`), so those could still be considered compatible.
        self.do_getset_subtest(
            collection, prop_rna, is_set,
