...
DefNode(CompositorNode, CMP_NODE_NORMAL,            0,                      "NORMAL",          Normal,           "Normal",              "Generates a normal vector and a dot product")
DefNode(CompositorNode, CMP_NODE_CURVE_VEC,         def_vector_curve,       "CURVE_VEC",       CurveVec,         "Vector Curves",       "Maps an input vector components to a curve")
DefNode(CompositorNode, CMP_NODE_CURVE_RGB,         def_rgb_curve,          "CURVE_RGB",       CurveRGB,         "RGB Curves",          "Perform level adjustments on each color channel of input image")
DefNode(CompositorNode, CMP_NODE_ALPHAOVER,         def_cmp_alpha_over,     "ALPHAOVER",       AlphaOver,        "Alpha Over",          "Overlay a foreground input image onto a background input image")
DefNode(CompositorNode, CMP_NODE_BLUR,              def_cmp_blur,           "BLUR",            Blur,             "Blur",                "Blur an image, providing several blur modes")
...
DefNode(CompositorNode, CMP_NODE_MAP_RANGE,         def_cmp_map_range,      "MAP_RANGE",       MapRange,         "Map Range",           "Convert (maps) an input value range into a destination range")
DefNode(CompositorNode, CMP_NODE_TIME,              def_time,               "TIME",            Time,             "Time Curve",          "Generate a factor value (from 0.0 to 1.0) that changes according to the curve as time progresses through the Timeline")
...
DefNode(CompositorNode, CMP_NODE_IMAGE,             def_cmp_image,          "IMAGE",           Image,            "Image",               "Import image or movie supported by Blender")
DefNode(CompositorNode, CMP_NODE_R_LAYERS,          def_cmp_render_layers,  "R_LAYERS",        RLayers,          "Render Layers",       "Input a picture of your scene")
DefNode(CompositorNode, CMP_NODE_COMPOSITE,         def_cmp_composite,      "COMPOSITE",       Composite,        "Composite",           "Final output node within the compositor workspace")
...
DefNode(CompositorNode, CMP_NODE_TRANSLATE,      def_cmp_translate,         "TRANSLATE",       Translate,        "Translate",           "Moves an input image")
DefNode(CompositorNode, CMP_NODE_ZCOMBINE,       def_cmp_zcombine,          "ZCOMBINE",        Zcombine,         "Z Combine",           "Combine two input images based on their Z-depth maps")
DefNode(CompositorNode, CMP_NODE_COMBRGBA_LEGACY,0,                         "COMBRGBA",        CombRGBA,         "Combine RGBA",        "Deprecated")
DefNode(CompositorNode, CMP_NODE_DILATEERODE,    def_cmp_dilate_erode,      "DILATEERODE",     DilateErode,      "Dilate/Erode",        "Apply mathematical shape analysis filter to mask, which allow expanding/shrinking masks")
...
DefNode(CompositorNode, CMP_NODE_COLOR_SPILL,    def_cmp_color_spill,       "COLOR_SPILL",     ColorSpill,       "Color Spill",         "Reduce one of the RGB channels so that it is not greater than any of the others")
DefNode(CompositorNode, CMP_NODE_CHROMA_MATTE,   def_cmp_chroma_matte,      "CHROMA_MATTE",    ChromaMatte,      "Chroma Key",          "Determines if a pixel is a foreground or background (and thereby should be transparent) based on its chroma values")
...
DefNode(CompositorNode, CMP_NODE_FLIP,           def_cmp_flip,              "FLIP",            Flip,             "Flip",                "Flips an image at defined axis")
DefNode(CompositorNode, CMP_NODE_SPLIT,          def_cmp_split,             "SPLIT",           Split,            "Split",               "Get two images and displays these side-by-side")
DefNode(CompositorNode, CMP_NODE_MAP_UV,         def_cmp_map_uv,            "MAP_UV",          MapUV,            "Map UV",              "Change the texture after they have been rendered")
DefNode(CompositorNode, CMP_NODE_ID_MASK,        def_cmp_id_mask,           "ID_MASK",         IDMask,           "ID Mask",             "Can be used to access an alpha mask per object or per material")
...
DefNode(CompositorNode, CMP_NODE_DEFOCUS,        def_cmp_defocus,           "DEFOCUS",         Defocus,          "Defocus",             "Blur areas of an image based on a map/mask input")
DefNode(CompositorNode, CMP_NODE_DISPLACE,       0,                         "DISPLACE",        Displace,         "Displace",            "Displaces the pixel position based on an input vector")
...
DefNode(CompositorNode, CMP_NODE_LUMA_MATTE,     def_cmp_luma_matte,        "LUMA_MATTE",      LumaMatte,        "Luminance Key",       "Determine background objects from foreground objects by the difference in the luminance (brightness) levels")
DefNode(CompositorNode, CMP_NODE_BRIGHTCONTRAST, def_cmp_brightcontrast,    "BRIGHTCONTRAST",  BrightContrast,   "Brightness/Contrast", "Control the brightness and contrast of the input image")
DefNode(CompositorNode, CMP_NODE_GAMMA,          0,                         "GAMMA",           Gamma,            "Gamma",               "Apply a gamma correction on the input image")
DefNode(CompositorNode, CMP_NODE_INVERT,         def_cmp_invert,            "INVERT",          Invert,           "Invert Color",        "Invert the colors in the input image, producing a negative")
DefNode(CompositorNode, CMP_NODE_NORMALIZE,      0,                         "NORMALIZE",       Normalize,        "Normalize",           "Find the minimum and maximum values of a single channel. Then map the values to a range of 0 and 1")
DefNode(CompositorNode, CMP_NODE_CROP,           def_cmp_crop,              "CROP",            Crop,             "Crop",                "Crops an input image to a selected region by either making the cropped area transparent or by resizing the input image")
DefNode(CompositorNode, CMP_NODE_DBLUR,          def_cmp_dblur,             "DBLUR",           DBlur,            "Directional Blur",    "Blur an input image based on specified origin")
DefNode(CompositorNode, CMP_NODE_BILATERALBLUR,  def_cmp_bilateral_blur,    "BILATERALBLUR",   Bilateralblur,    "Bilateral Blur",      "Perform a high-quality adaptive blur on the source image, allowing to blur images while retaining their sharp edges")
DefNode(CompositorNode, CMP_NODE_PREMULKEY,      def_cmp_premul_key,        "PREMULKEY",       PremulKey,        "Alpha Convert",       "Convert the alpha channel interpretation of an input image")
DefNode(CompositorNode, CMP_NODE_GLARE,          def_cmp_glare,             "GLARE",           Glare,            "Glare",               "Add lens flares, fog, glows around exposed parts of an input image")
DefNode(CompositorNode, CMP_NODE_TONEMAP,        def_cmp_tonemap,           "TONEMAP",         Tonemap,          "Tonemap",             "Map one set of colors to another in order to approximate the appearance of high dynamic range in input image")
DefNode(CompositorNode, CMP_NODE_LENSDIST,       def_cmp_lensdist,          "LENSDIST",        Lensdist,         "Lens Distortion",     "Simulate distortions that real camera lenses produce, like distorsion and dispersion")
DefNode(CompositorNode, CMP_NODE_VIEW_LEVELS,    def_cmp_levels,            "LEVELS",          Levels,           "Levels",              "Read the individual color channels of its input and outputs the resulting value such as average and standard deviation")
DefNode(CompositorNode, CMP_NODE_COLOR_MATTE,    def_cmp_color_matte,       "COLOR_MATTE",     ColorMatte,       "Color Key",           "Create a matte based on a specified color of the input image")
DefNode(CompositorNode, CMP_NODE_DIST_MATTE,     def_cmp_distance_matte,    "DISTANCE_MATTE",  DistanceMatte,    "Distance Key",        "Determines a pixel’s alpha value based on the three-dimensional distance between the image pixel color and the key color in a 3D color space")
DefNode(CompositorNode, CMP_NODE_COLORBALANCE,   def_cmp_colorbalance,      "COLORBALANCE",    ColorBalance,     "Color Balance",       "Adjust the color and values of an input image")
DefNode(CompositorNode, CMP_NODE_HUECORRECT,     def_cmp_huecorrect,        "HUECORRECT",      HueCorrect,       "Hue Correct",         "Adjust the Hue, Saturation, and Value of an input image, with an input curve")
DefNode(CompositorNode, CMP_NODE_MOVIECLIP,      def_cmp_movieclip,         "MOVIECLIP",       MovieClip,        "Movie Clip",          "Import movie/image from file or from editor")
DefNode(CompositorNode, CMP_NODE_TRANSFORM,      dev_cmd_transform,         "TRANSFORM",       Transform,        "Transform",           "Combination of Scaling, Translating and Rotate nodes")
DefNode(CompositorNode, CMP_NODE_STABILIZE2D,    def_cmp_stabilize2d,       "STABILIZE2D",     Stabilize,        "Stabilize 2D",        "Stabilize the footage according to the settings set in Movie Clip Editor ‣ Properties ‣ Stabilization ‣ 2D Stabilization")
DefNode(CompositorNode, CMP_NODE_MOVIEDISTORTION,def_cmp_moviedistortion,   "MOVIEDISTORTION", MovieDistortion,  "Movie Distortion",    "Remove distortion from movies or add distortion to render to make our render blend in with the movie clip")
...
DefNode(CompositorNode, CMP_NODE_SWITCH_VIEW,    def_cmp_switch_view,       "VIEWSWITCH",      SwitchView,       "Switch View",         "Combines the views (left and right) into a single Stereo 3D output")
DefNode(CompositorNode, CMP_NODE_COLORCORRECTION,def_cmp_colorcorrection,   "COLORCORRECTION", ColorCorrection,  "Color Correction",    "Adjust the color of an image, separately in several tonal ranges (highlights, midtones and shadows) and only affect the necessary RGB channels")
DefNode(CompositorNode, CMP_NODE_MASK,           def_cmp_mask,              "MASK",            Mask,             "Mask",                "Input mask from Image Editor")
...
DefNode(CompositorNode, CMP_NODE_TRACKPOS,       def_cmp_trackpos,          "TRACKPOS",        TrackPos,         "Track Position",      "Provide information about tracking points, such as x and y values")
DefNode(CompositorNode, CMP_NODE_PIXELATE,       def_cmp_pixelate,          "PIXELATE",        Pixelate,         "Pixelate",            "Reduce the level of detail in an image by causing individual pixels to become more prominent. It results in a blocky or mosaic-like appearance")
DefNode(CompositorNode, CMP_NODE_PLANETRACKDEFORM,def_cmp_planetrackdeform,"PLANETRACKDEFORM", PlaneTrackDeform, "Plane Track Deform",  "Incorporate the special “plane track” in your composite by checking areas which are planes, and replacing their footage with some other image")
DefNode(CompositorNode, CMP_NODE_CORNERPIN,      0,                         "CORNERPIN",       CornerPin,        "Corner Pin",          "Uses explicit corner values for a plane warp transformation")
DefNode(CompositorNode, CMP_NODE_SUNBEAMS,       def_cmp_sunbeams,          "SUNBEAMS",        SunBeams,         "Sun Beams",           "Create sun beam effect based on the image brightness alone")
DefNode(CompositorNode, CMP_NODE_CRYPTOMATTE,    def_cmp_cryptomatte,       "CRYPTOMATTE_V2",  CryptomatteV2,    "Cryptomatte",         "Create mattes for compositing from image or render")
...
DefNode(CompositorNode, CMP_NODE_EXPOSURE,       0,                         "EXPOSURE",        Exposure,         "Exposure",            "Adjust the perceived brightness of an image by a scalar value")
DefNode(CompositorNode, CMP_NODE_ANTIALIASING,   def_cmp_antialiasing,      "ANTIALIASING",    AntiAliasing,     "Anti-Aliasing",       "Remove distortion artifacts around edges known as aliasing")
DefNode(CompositorNode, CMP_NODE_POSTERIZE,      0,                         "POSTERIZE",       Posterize,        "Posterize",           "Reduce the number of colors that compose the image by converting portions of continuous gradation into abrupt changes from one color to another")
DefNode(CompositorNode, CMP_NODE_CONVERT_COLOR_SPACE,def_cmp_convert_color_space,"CONVERT_COLORSPACE",ConvertColorSpace,"Convert Colorspace","Converts images from one color spaces")
DefNode(CompositorNode, CMP_NODE_SCENE_TIME,     0,                         "SCENE_TIME",      SceneTime,        "Scene Time",          "Output the current time in the scene’s animation in units of seconds or frames")
DefNode(CompositorNode, CMP_NODE_COMBINE_XYZ,    0,                         "COMBINE_XYZ",     CombineXYZ,       "Combine XYZ",         "Combines a vector from its individual components")
DefNode(CompositorNode, CMP_NODE_SEPARATE_XYZ,   0,                         "SEPARATE_XYZ",    SeparateXYZ,      "Separate XYZ",        "Splits a vector into its individual components")
...
DefNode(CompositorNode, CMP_NODE_KUWAHARA,       def_cmp_kuwahara,          "KUWAHARA",        Kuwahara,         "Kuwahara",            "Apply smoothing filter that tries to preserve the edges in the input image")
