...
{
  b.add_input<decl::Vector>("Min").default_value(float3(-1.0f)).description("Minimum boundary of volume");
  b.add_input<decl::Vector>("Max").default_value(float3(1.0f)).description("Maximum boundary of volume");

  b.add_input<decl::Int>("Resolution X").default_value(32).min(2);
  b.add_input<decl::Int>("Resolution Y").default_value(32).min(2);

  b.add_input<decl::Bool>("Bitmap").description("True or false is a pixel").supports_field().hide_value();
...
class ImageBitMapFieldContext : public FieldContext {
 ...
 public:
  ImageBitMapFieldContext(const int2 resolution, const float2 min_point, const float2 max_point) : resolution_(resolution), min_point_(min_point), max_point_(max_point) {}
                               ...
                               ResourceScope & /*scope*/) const
  {
    ...
    threading::parallel_for(IndexRange(resolution_.y), 4096, [&](const IndexRange range) {
      for (const int y_index : range) {
        int64_t start_offset = y_index * resolution_.x;
        const float y_factor = bke::attribute_math::mix2(float(y_index) / float(resolution_.y - 1), min_point_[1], max_point_[1]);
        for (const int64_t x_index : IndexRange(resolution_.x)) {
          const float x_factor = bke::attribute_math::mix2(float(x_index) / float(resolution_.x - 1), min_point_[0], max_point_[0]);
          positions[start_offset + x_index] = float3(x_factor, y_factor, 0.0f);
        }
      }
    }, threading::accumulated_task_sizes([&](const IndexRange range) { return range.size() * resolution_.x; }));
...
static void node_geo_exec(GeoNodeExecParams params)
{
  ...
  const int2 resolution = int2(params.extract_input<int>("Resolution X"), params.extract_input<int>("Resolution Y"));
  ...
  AnonymousAttributeIDPtr parent_curve_id = params.get_output_anonymous_attribute_id_if_needed("Parent Curve");
  Curves *curve = geometry::plane_to_curve(resolution, byte_map, min_point, max_point, parent_curve_id.get());
  if (curve == nullptr) {
