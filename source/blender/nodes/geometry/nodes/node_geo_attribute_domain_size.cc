...
static void node_declare(NodeDeclarationBuilder &b)
{
  ...
  b.add_output<decl::Int>("Point Count").make_available([](bNode &node) {
    node.custom1 = bke::GEO_COMPONENT_TYPE_MESH;
  });
  ...
  b.add_output<decl::Int>("Spline Count").make_available([](bNode &node) {
    node.custom1 = bke::GEO_COMPONENT_TYPE_CURVE;
  });
  b.add_output<decl::Int>("Instance Count").make_available([](bNode &node) {
    node.custom1 = bke::GEO_COMPONENT_TYPE_INSTANCES;
  });
}
...
static void node_update(bNodeTree *ntree, bNode *node)
{
  ...
  bke::nodeSetSocketAvailability(
      ntree, edge_socket, node->custom1 == bke::GEO_COMPONENT_TYPE_MESH);
  bke::nodeSetSocketAvailability(
      ntree, face_socket, node->custom1 == bke::GEO_COMPONENT_TYPE_MESH);
  bke::nodeSetSocketAvailability(
      ntree, face_corner_socket, node->custom1 == bke::GEO_COMPONENT_TYPE_MESH);
  bke::nodeSetSocketAvailability(
      ntree, spline_socket, node->custom1 == bke::GEO_COMPONENT_TYPE_CURVE);
  bke::nodeSetSocketAvailability(
      ntree, instances_socket, node->custom1 == bke::GEO_COMPONENT_TYPE_INSTANCES);
}

static void node_geo_exec(GeoNodeExecParams params)
{
  const bke::GeometryComponentType component = bke::GeometryComponentType(params.node().custom1);
