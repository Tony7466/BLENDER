...
namespace blender::nodes::node_geo_erode_grid_cc {
                           ...
                           const GeometryNodeGridNeighborTopology neighbors_mode)
{
  ...
  openvdb::tools::erodeActiveValues(grid.grid_for_write(tree_token).tree(),
                                    iterations,
...
static void node_geo_exec(GeoNodeExecParams params)
{
  ...
  switch (data_type) {
    case CD_PROP_FLOAT:
      try_erode_grid<float>(params, iterations, neighbors_mode);
      break;
    case CD_PROP_FLOAT3:
      try_erode_grid<float3>(params, iterations, neighbors_mode);
      break;
    default:
