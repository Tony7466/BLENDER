    ...
    const Map<AttributeIDRef, AttributeKind> &attributes_to_propagate)
{
  ...
  selection.foreach_index(
      GrainSize(1024), [&](const int64_t selection_index, const int64_t position) {
        /* Compute base transform for every instances. */
        float4x4 &dst_transform = dst_transforms[position];
        dst_transform = math::from_loc_rot_scale<float4x4>(
            positions[selection_index],
            math::EulerXYZ(rotations[selection_index]),
            scales[selection_index]);

        /* Reference that will be used by this new instance. */
        int dst_handle = empty_reference_handle;

        const bool use_individual_instance = pick_instance[selection_index];
        if (use_individual_instance) {
          if (src_instances != nullptr) {
            const int src_instances_num = src_instances->instances_num();
            const int original_index = indices[selection_index];
        ...
        dst_handles[position] = dst_handle;
      });
