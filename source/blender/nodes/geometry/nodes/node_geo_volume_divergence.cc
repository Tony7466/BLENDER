...
namespace blender::nodes::node_geo_volume_divergence_cc {
...
static void node_declare(NodeDeclarationBuilder &b)
{
  ...
  b.add_input<decl::Vector>(N_("Vector")).supports_field();

  b.add_output<decl::Geometry>(CTX_N_(BLT_I18NCONTEXT_ID_ID, "Volume"))
      .translation_context(BLT_I18NCONTEXT_ID_ID)
      .supported_type(GeometryComponent::Type::Volume);
  b.add_output<decl::Float>(N_("Value")).dependent_field({1});
}
...
void register_node_type_geo_volume_divergence()
{
  namespace file_ns = blender::nodes::node_geo_volume_divergence_cc;

  static bNodeType ntype;

  geo_node_type_base(&ntype, GEO_NODE_VOLUME_DIVERGENCE, "Divergence", NODE_CLASS_CONVERTER);
