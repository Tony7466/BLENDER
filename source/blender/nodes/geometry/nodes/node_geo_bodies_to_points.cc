...
namespace blender::nodes::node_geo_bodies_to_points_cc {

static void node_declare(NodeDeclarationBuilder &b)
{
  b.add_input<decl::Geometry>("Bodies").supported_type(bke::GeometryComponent::Type::Physics);
...
static void convert_bodies_to_points(GeometrySet &geometry_set,
                                     ...
                                     const AttributeFilter &attribute_filter)
{
  ...
  const bke::PhysicsFieldContext context{physics, AttrDomain::Point};
  ...
  for (const auto item : attributes_to_propagate.items()) {
    ...
    if (selection.size() == physics.bodies_num() && src.sharing_info && src.varray.is_span()) {
...
{
  GeometrySet geometry_set = params.extract_input<GeometrySet>("Bodies");

  if (geometry_set.has_physics()) {
    convert_bodies_to_points(geometry_set,
    ...
    geometry_set.keep_only({GeometryComponent::Type::PointCloud});
