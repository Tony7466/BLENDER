  ...
  for (const bNode *bnode : context.btree_->all_nodes()) {
    if (bnode->is_group()) {
      ...
      if (child_btree != nullptr) {
        const bNodeInstanceKey child_key = blender::bke::node_instance_key(instance_key, &btree, bnode);
...
{
  return blender::bke::node_instance_key(context()->instance_key(), &context()->btree(), bnode());
}
