...
static void sh_node_mix_update(bNodeTree *ntree, bNode *node)
{
  ...
  blender::bke::nodeSetSocketAvailability(ntree, sock_factor, !use_vector_factor);

  blender::bke::nodeSetSocketAvailability(ntree, sock_factor_vec, use_vector_factor);

  for (bNodeSocket *socket = sock_factor_vec->next; socket != nullptr; socket = socket->next) {
    blender::bke::nodeSetSocketAvailability(ntree, socket, socket->type == data_type);
  }
