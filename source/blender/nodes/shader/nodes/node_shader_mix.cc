...
static void sh_node_mix_declare(NodeDeclarationBuilder &b)
{
  ...
  b.add_input<decl::Float>("Factor", "Factor_Float")
      ...
      .no_muted_links()
      .description(
          "Controls the amount of mixing between the first and second inputs.\n"
          "At zero it uses the first input entirely and at one the second input");
  b.add_input<decl::Vector>("Factor", "Factor_Vector")
      .default_value(float3(0.5f))
      .subtype(PROP_FACTOR)
      .no_muted_links()
      .description(
          "Controls the amount of mixing between the Vector1 and Vector2 inputs"
          "At zero it uses the first vector entirely and at one the second vector");

  b.add_input<decl::Float>("Float1", "A_Float")
      .min(-10000.0f)
      .max(10000.0f)
      .is_default_link_socket()
      .translation_context(BLT_I18NCONTEXT_ID_NODETREE);
  b.add_input<decl::Float>("Float2", "B_Float")
      .min(-10000.0f)
      .max(10000.0f)
      .translation_context(BLT_I18NCONTEXT_ID_NODETREE);

  b.add_input<decl::Vector>("Vector1", "A_Vector")
      .is_default_link_socket()
      .translation_context(BLT_I18NCONTEXT_ID_NODETREE);
  b.add_input<decl::Vector>("Vector2", "B_Vector")
      .translation_context(BLT_I18NCONTEXT_ID_NODETREE);

  b.add_input<decl::Color>("Color1", "A_Color")
      .default_value({0.5f, 0.5f, 0.5f, 1.0f})
      .is_default_link_socket()
      .translation_context(BLT_I18NCONTEXT_ID_NODETREE);
  b.add_input<decl::Color>("Color2", "B_Color")
      .default_value({0.5f, 0.5f, 0.5f, 1.0f})
      .translation_context(BLT_I18NCONTEXT_ID_NODETREE);

  b.add_input<decl::Rotation>("Rotation1", "A_Rotation")
      .is_default_link_socket()
      .translation_context(BLT_I18NCONTEXT_ID_NODETREE);
  b.add_input<decl::Rotation>("Rotation2", "B_Rotation")
      .translation_context(BLT_I18NCONTEXT_ID_NODETREE);
