...
static void sh_node_tex_noise_declare(NodeDeclarationBuilder &b)
{
  ...
  b.add_input<decl::Float>("Detail").min(0.0f).max(15.0f).default_value(2.0f)
      .description("The number of noise octaves. Higher values give more detailed noise but increase render time");
  b.add_input<decl::Float>("Roughness")
      .min(0.0f)
      .max(1.0f)
      .default_value(0.5f)
      .subtype(PROP_FACTOR)
      .description("Blend factor between an octave and its previous one. A value of zero corresponds to zero detail");
  ...
  b.add_input<decl::Float>("Offset").min(-1000.0f).max(1000.0f).default_value(0.0f).make_available(
      ...
      .description("An added offset to each octave, determines the level where the highest octave will appear");
