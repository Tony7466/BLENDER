...
{
  bNodeSocket *sock_rotation = blender::bke::nodeFindSocket(node, SOCK_IN, "Rotation");
  bke::nodeSetSocketAvailability(
      ntree, sock_rotation, ELEM(node->custom1, NODE_VECTOR_ROTATE_TYPE_EULER_XYZ));
  bNodeSocket *sock_axis = blender::bke::nodeFindSocket(node, SOCK_IN, "Axis");
  bke::nodeSetSocketAvailability(
      ntree, sock_axis, ELEM(node->custom1, NODE_VECTOR_ROTATE_TYPE_AXIS));
  bNodeSocket *sock_angle = blender::bke::nodeFindSocket(node, SOCK_IN, "Angle");
  bke::nodeSetSocketAvailability(
