...
enum class InputSocketFieldType {
  ...
  IsSupported,
  /** The input can be a field and is a field implicitly if nothing is
     connected. */
  Implicit,
};
...
/**
 * Contains information about how a node output's field state depends on inputs
 * of the same node.
 */
...
public:
  ...
  friend bool operator==(const OutputFieldDependency &a,
...
struct PropagateRelation {
  ...
  friend bool operator==(const PropagateRelation &a,
                         const PropagateRelation &b) {
...
/**
 * References to attributes can be propagated from an input field to an output
 * field.
 */
struct ReferenceRelation {
  ...
  friend bool operator==(const ReferenceRelation &a,
                         const ReferenceRelation &b) {
    return a.from_field_input == b.from_field_input &&
           a.to_field_output == b.to_field_output;
  }
};
...
struct EvalRelation {
  ...
  friend bool operator==(const EvalRelation &a, const EvalRelation &b) {
    return a.field_input == b.field_input &&
           a.geometry_input == b.geometry_input;
  }
};
...
struct AvailableRelation {
  ...
  friend bool operator==(const AvailableRelation &a,
                         const AvailableRelation &b) {
    return a.field_output == b.field_output &&
           a.geometry_output == b.geometry_output;
  }
};
...
std::ostream &operator<<(std::ostream &stream,
                         const RelationsInNode &relations);

} // namespace anonymous_attribute_lifetime
namespace aal = anonymous_attribute_lifetime;

using ImplicitInputValueFn =
    std::function<void(const bNode &node, void *r_value)>;
...
/**
 * Describes a single input or output socket. This is subclassed for different
 * socket types.
 */
...
private:
  ...
  /** This input expects a single value and can't operate on non-single values.
   * See realtime_compositor::InputDescriptor for more information. */
  bool compositor_expects_single_value_ = false;

  /** Utility method to make the socket available if there is a straightforward
   * way to do so. */
  std::function<void(bNode &)> make_available_fn_;

  /** Some input sockets can have non-trivial values in the case when they are
   * unlinked. This
   * callback computes the default input of a values in geometry nodes when
   * nothing is linked. */
...
public:
  ...
  virtual bNodeSocket &update_or_build(bNodeTree &ntree, bNode &node,
                                       bNodeSocket &socket) const;

  /**
   * Determine if a new socket described by this declaration could have a valid
   * connection the other socket.
   */
  virtual bool can_connect(const bNodeSocket &socket) const = 0;

  /**
   * Change the node such that the socket will become visible. The node type's
   * update method should be called afterwards. \note this is not necessarily
   * implemented for all node types.
   */
  void make_available(bNode &node) const;

  const CompositorInputRealizationOptions &
  compositor_realization_options() const;
 ...
 * This is separate from #SocketDeclaration, because it allows separating the
 * API used by nodes to declare themselves from how the declaration is stored
 * internally.
 */
...
public:
  ...
  /**
   * Used for sockets that are always unavailable and should not be seen by the
   * user. Ideally, no new calls to this method should be added over time.
   */
  ...
  /**
   * For inputs this means that the input field is evaluated on all geometry
   * inputs. For outputs it means that this contains an anonymous attribute
   * reference that is available on all geometry outputs. This sockets value
   * does not have to be output manually in the node. It's done automatically by
   * #LazyFunctionForGeometryNode. This allows outputting this field even if the
  ...
  /** For inputs that are evaluated or available on a subset of the geometry
   * sockets. */
  Self &field_on(Span<int> indices);

  /** The input supports a field and is a field by default when nothing is
   * connected. */
  Self &implicit_field(ImplicitInputValueFn fn) {
    this->hide_value();
    if (decl_in_) {
      decl_in_->input_field_type = InputSocketFieldType::Implicit;
      decl_in_->implicit_input_fn_ =
          std::make_unique<ImplicitInputValueFn>(std::move(fn));
    }
    return *(Self *)this;
  }
  ...
  Self &implicit_field_on(ImplicitInputValueFn fn,
  ...
  Self &field_source() {
    if (decl_out_) {
      decl_out_->output_field_dependency =
          OutputFieldDependency::ForFieldSource();
    }
    return *(Self *)this;
  }
  ...
  /** The output is a field if any of the inputs with indices in the given list
   * is a field. */
  ...
  /**
   * For outputs that combine all input fields into a new field. The output is a
   * field even if none of the inputs is a field.
   */
  ...
  Self &
  compositor_realization_options(CompositorInputRealizationOptions value) {
    if (decl_in_) {
  ...
  /**
   * Pass a function that sets properties on the node required to make the
   * corresponding socket available, if it is not available on the default state
   * of the node. The function is allowed to make other sockets unavailable,
   * since it is meant to be called when the node is first added. The node
   * type's update function is called afterwards.
   */
...
protected:
  SocketDeclaration *input_declaration() override { return decl_in_; }
  SocketDeclaration *output_declaration() override { return decl_out_; }
};
...
public:
  ...
  /** Leave the sockets in place, even if they don't match the declaration. Used
   * for dynamic declarations when the information used to build the declaration
   * is missing, but might become available again in the future. */
  bool skip_updating_sockets = false;

  /** Use order of socket declarations for socket order instead of conventional
   * outputs | buttons | inputs order. Panels are only supported when using
   * custom socket order. */
...
public:
  ...
  /**
   * All inputs support fields, and all outputs are fields if any of the inputs
   * is a field. Calling field status definitions on each socket is unnecessary.
   */
  ...
  aal::RelationsInNode &get_anonymous_attribute_relations() {
    if (!declaration_.anonymous_attribute_relations_) {
      declaration_.anonymous_attribute_relations_ =
          std::make_unique<aal::RelationsInNode>();
    }
...
private:
  ...
  add_socket(StringRef name, StringRef identifier_in, StringRef identifier_out,
             eNodeSocketInOut in_out);
...
template <typename SocketDecl>
typename SocketDeclarationBuilder<SocketDecl>::Self &
SocketDeclarationBuilder<SocketDecl>::reference_pass(
    const Span<int> input_indices) {
...
template <typename SocketDecl>
typename SocketDeclarationBuilder<SocketDecl>::Self &
SocketDeclarationBuilder<SocketDecl>::field_on(const Span<int> indices) {
  aal::RelationsInNode &relations =
      node_decl_builder_->get_anonymous_attribute_relations();
  if (decl_in_) {
    this->supports_field();
...
inline OutputFieldDependency
OutputFieldDependency::ForPartiallyDependentField(Vector<int> indices) {
  OutputFieldDependency field_dependency;
  if (indices.is_empty()) {
    field_dependency.type_ = OutputSocketFieldType::None;
  } else {
...
inline bool operator==(const OutputFieldDependency &a,
                       const OutputFieldDependency &b) {
  return a.type_ == b.type_ &&
         a.linked_input_indices_ == b.linked_input_indices_;
}
...
inline bool operator==(const FieldInferencingInterface &a,
...
inline const CompositorInputRealizationOptions &
SocketDeclaration::compositor_realization_options() const {
...
template <typename DeclType>
typename DeclType::Builder &
PanelDeclarationBuilder::add_input(StringRef name, StringRef identifier) {
  if (is_complete_) {
  ...
  return node_decl_builder_->add_socket<DeclType>(name, identifier, "",
                                                  SOCK_IN);
}

template <typename DeclType>
typename DeclType::Builder &
PanelDeclarationBuilder::add_output(StringRef name, StringRef identifier) {
  if (is_complete_) {
    static typename DeclType::Builder dummy_builder = {};
    BLI_assert_unreachable();
    return dummy_builder;
...
template <typename DeclType>
typename DeclType::Builder &PanelDeclarationBuilder::add_input_output(
...
template <typename DeclType>
inline typename DeclType::Builder &
NodeDeclarationBuilder::add_input(StringRef name, StringRef identifier) {
...
template <typename DeclType>
inline typename DeclType::Builder &
NodeDeclarationBuilder::add_output(StringRef name, StringRef identifier) {
  set_active_panel_builder(nullptr);
...
inline typename DeclType::Builder &NodeDeclarationBuilder::add_input_output(
    StringRef name, StringRef identifier_in, StringRef identifier_out) {
  set_active_panel_builder(nullptr);
  return this->add_socket<DeclType>(name, identifier_in, identifier_out,
                                    SOCK_IN | SOCK_OUT);
}

template <typename DeclType>
inline typename DeclType::Builder &
NodeDeclarationBuilder::add_socket(StringRef name, StringRef identifier_in,
                                   StringRef identifier_out,
                                   eNodeSocketInOut in_out) {
  ...
  if (in_out & SOCK_IN) {
    ...
    socket_decl_builder->index_in_ =
        declaration_.inputs.append_and_get_index(socket_decl.get());
  ...
  if (in_out & SOCK_OUT) {
    ...
    socket_decl_builder->index_out_ =
        declaration_.outputs.append_and_get_index(socket_decl.get());
    declaration_.items.append(std::move(socket_decl));
  }
...
inline Span<SocketDeclaration *>
NodeDeclaration::sockets(eNodeSocketInOut in_out) const {
  if (in_out == SOCK_IN) {
    return inputs;
  }
  return outputs;
