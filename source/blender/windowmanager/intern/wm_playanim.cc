                                      ...
                                      const int frame_offset)
{
  ...
  ImBuf_Anim *anim = IMB_open_anim(filepath_first, IB_rect, 0, nullptr);
  if (anim == nullptr) {
...
static bool wm_main_playanim_intern(int argc, const char **argv, PlayArgs *args_next)
{
  ...
  if (IMB_isanim(filepath)) {
    ...
    ImBuf_Anim *anim = IMB_open_anim(filepath, IB_rect, 0, nullptr);
    if (anim) {
  ...
  if (!BLI_listbase_is_empty(&ps.picsbase)) {
    ImBuf_Anim *anim_movie = static_cast<PlayAnimPict *>(ps.picsbase.first)->anim;
    if (anim_movie) {
      short frs_sec = 25;
