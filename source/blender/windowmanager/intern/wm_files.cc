                                                ...
                                                const int dst_size[2])
{
  ...
  uint8_t *dst_rect = (uint8_t *)MEM_mallocN(sizeof(uint8_t[4]) * dst_size[0] * dst_size[1],
                                             __func__);

  /* A row, the width of the destination to accumulate pixel values into
   * before writing into the image. */
  uint32_t *accum_row = (uint32_t *)MEM_callocN(sizeof(uint32_t) * dst_size[0] * 4, __func__);
