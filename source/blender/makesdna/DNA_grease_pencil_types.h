...
typedef struct GreasePencil {
   ...
   * \param duration: Duration of the keyframe. If 0, then the keyframe is implicitly held (until
   * the next keyframe). \param keytype: The keyframe type (used to render different colored keys).
   *
   * \returns A pointer to a drawing if the keyframe was inserted, otherwise nullptr.
   */
  blender::bke::greasepencil::Drawing *insert_frame(blender::bke::greasepencil::Layer &layer,
                                                    int frame_number,
                                                    int duration,
