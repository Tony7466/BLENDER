...
TEST_F(AnimationLayersTest, add_output)
{
  ...
  EXPECT_EQ(1, anim->last_output_handle);
  EXPECT_EQ(1, out.handle);

  EXPECT_STREQ("", out.name);
  EXPECT_EQ(0, out.idtype);
}

TEST_F(AnimationLayersTest, rename_output_name_collision)
{
  Output &out1 = anim->output_add();
  Output &out2 = anim->output_add();

  anim->output_name_set(out1, "New Output Name");
  anim->output_name_set(out2, "New Output Name");
  EXPECT_STREQ("New Output Name", out1.name);
  EXPECT_STREQ("New Output Name.001", out2.name);
}
