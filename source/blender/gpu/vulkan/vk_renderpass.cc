...
namespace blender::gpu {
  ...
  const static VkSubpassDependency2
      dependencies[static_cast<uint64_t>(DependencyType::DEPENDENCY_TYPE_ALL)] = {
...
void VKRenderPass::dependency_set(bool use_depth)
{
  ...
  switch (render_pass_enum_) {
    case VKRenderPassTransition::A2A:
      switch (depth_only) {
        case true:
          vk_create_info_[info_id_].dependencyCount = 1;
          vk_create_info_[info_id_].pDependencies = &vk_subpass::dependencies[static_cast<uint64_t>(vk_subpass::DependencyType::BASIC_DEPTH)];
          break;
        case false:
          vk_create_info_[info_id_].pDependencies =
              &vk_subpass::dependencies[static_cast<uint64_t>(vk_subpass::DependencyType::BASIC_COLOR)];
          switch (use_depth) {
            case true:
      ...
      break;
    case VKRenderPassTransition::S2S:
      switch (depth_only) {
        case true:
          vk_create_info_[info_id_].dependencyCount = 3;
          vk_create_info_[info_id_].pDependencies =
              &vk_subpass::dependencies[static_cast<uint64_t>(vk_subpass::DependencyType::SHADER_READ_DEPATH_BEGIN)];
          break;
        case false:
          vk_create_info_[info_id_].pDependencies =
              &vk_subpass::dependencies[static_cast<uint64_t>(vk_subpass::DependencyType::SHADER_READ_BEGIN)];
          switch (use_depth) {
            case true:
...
void VKRenderPass::multiview_set()
{
  ...
  if (subpass_[info_id_].viewMask !=
      subpass_[info_id_counter()].viewMask)
  {
