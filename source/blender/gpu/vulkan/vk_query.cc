...
VKQueryPool::~VKQueryPool()
{
  ...
  const VKDevice &device = backend.device_get();
  VK_ALLOCATION_CALLBACKS;

  while (!vk_query_pools_.is_empty()) {
    VkQueryPool vk_query_pool = vk_query_pools_.pop_last();
    vkDestroyQueryPool(device.device_get(), vk_query_pool, vk_allocation_callbacks);
  }
}
...
void VKQueryPool::begin_query()
{
  if (queries_issued_ == queries_allocated_) {
    ...
    vkCreateQueryPool(device.device_get(), &create_info, vk_allocation_callbacks, &vk_query_pool);
  ...
  const uint32_t query_index = query_index_in_pool();
  command_buffer.begin_query(vk_query_pool, query_index);
}
...
void VKQueryPool::end_query()
{
  ...
  VkQueryPool vk_query_pool = vk_query_pools_.last();
  const uint32_t query_index = query_index_in_pool();
  command_buffer.end_query(vk_query_pool, query_index);
  queries_issued_ += 1;
}
...
void VKQueryPool::get_occlusion_result(MutableSpan<uint32_t> r_values)
{
  ...
  for (int index : IndexRange(vk_query_pools_.size() - 1)) {
    ...
    vkGetQueryPoolResults(device.device_get(),
                          vk_query_pool,
                          0,
                          query_chunk_len_,
