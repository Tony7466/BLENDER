        ...
        if (a->comp_len == 16 || a->comp_len == 12 || a->comp_len == 8) {
          ...
          if (a->comp_len == 16) {
            ...
            for (int i = 0; i < a->comp_len / 4; i++) {
              ...
              desc.vertex_descriptor.attributes[mtl_attr.location + i].offset = attribute_offset +
                                                                                i * 16;
              /* All source data for a matrix is in the same singular buffer. */
              desc.vertex_descriptor.attributes[mtl_attr.location + i].buffer_index = buffer_index;
            ...
            MTL_LOG_INFO("Float4x4 attribute type added for '%s' at attribute locations: %d to %d",
                         name,
                         mtl_attr.location,
                         mtl_attr.location + 3);
          }
          ...
          BLI_assert(desc.vertex_descriptor.max_attribute_value < MTL_MAX_VERTEX_INPUT_ATTRIBUTES);
        }
        else {
