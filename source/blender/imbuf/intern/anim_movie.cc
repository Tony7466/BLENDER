...
static int startmovie(ImBuf_Anim * /*anim*/)
{
  return 1;
}
static ImBuf *movie_fetchibuf(ImBuf_Anim * /*anim*/, int /*position*/)
{
  return nullptr;
}
static void free_anim_movie(ImBuf_Anim * /*anim*/)
{
  /* pass */
}

#ifdef WITH_AVI
static void free_anim_avi(ImBuf_Anim *anim)
{
#  if defined(_WIN32)
  int i;
#  endif
...
#ifdef WITH_FFMPEG
static void free_anim_ffmpeg(ImBuf_Anim *anim);
#endif
...
IDProperty *IMB_anim_load_metadata(ImBuf_Anim *anim)
{
  switch (anim->curtype) {
    case ANIM_FFMPEG: {
#ifdef WITH_FFMPEG
                          ...
                          char colorspace[IM_MAX_SPACE])
{
  ...
  anim = (ImBuf_Anim *)MEM_callocN(sizeof(ImBuf_Anim), "anim struct");
  if (anim != nullptr) {
    if (colorspace) {
...
#ifdef WITH_AVI
static int startavi(ImBuf_Anim *anim)
{
...
#ifdef WITH_AVI
static ImBuf *avi_fetchibuf(ImBuf_Anim *anim, int position)
{
  ImBuf *ibuf = nullptr;
  int *tmp;
  int y;
...
static int startffmpeg(ImBuf_Anim *anim)
{
...
static double ffmpeg_steps_per_frame_get(ImBuf_Anim *anim)
{
 ...
 */
static void ffmpeg_double_buffer_backup_frame_store(ImBuf_Anim *anim, int64_t pts_to_search)
{
...
static void ffmpeg_double_buffer_backup_frame_clear(ImBuf_Anim *anim)
{
  if (anim->pFrame_backup_complete) {
    av_frame_unref(anim->pFrame_backup);
  }
...
static AVFrame *ffmpeg_double_buffer_frame_fallback_get(ImBuf_Anim *anim)
{
 ...
 */
static void ffmpeg_postprocess(ImBuf_Anim *anim, AVFrame *input, ImBuf *ibuf)
{
  int filter_y = 0;
...
static void final_frame_log(ImBuf_Anim *anim,
...
static AVFrame *ffmpeg_frame_by_pts_get(ImBuf_Anim *anim, int64_t pts_to_search)
{
...
static void ffmpeg_decode_store_frame_pts(ImBuf_Anim *anim)
{
...
static int ffmpeg_read_video_frame(ImBuf_Anim *anim, AVPacket *packet)
{
  int ret = 0;
...
static int ffmpeg_decode_video_frame(ImBuf_Anim *anim)
{
...
static int64_t ffmpeg_get_seek_pts(ImBuf_Anim *anim, int64_t pts_to_search)
{
 ...
 */
static int64_t ffmpeg_get_pts_to_search(ImBuf_Anim *anim, ImBuf_AnimIndex *tc_index, int position)
{
...
static bool ffmpeg_is_first_frame_decode(ImBuf_Anim *anim)
{
  return anim->pFrame_complete == false;
}

static void ffmpeg_scan_log(ImBuf_Anim *anim, int64_t pts_to_search)
{
...
static void ffmpeg_decode_video_frame_scan(ImBuf_Anim *anim, int64_t pts_to_search)
{
...
static int ffmpeg_generic_seek_workaround(ImBuf_Anim *anim,
 ...
 * position state. */
static void ffmpeg_seek_recover_stream_position(ImBuf_Anim *anim)
{
...
static bool ffmpeg_seek_buffers_need_flushing(ImBuf_Anim *anim, int position, int64_t seek_pos)
{
...
static int ffmpeg_seek_to_key_frame(ImBuf_Anim *anim,
                                    int position,
                                    ImBuf_AnimIndex *tc_index,
                                    int64_t pts_to_search)
{
  int64_t seek_pos;
  int ret;
...
static bool ffmpeg_must_seek(ImBuf_Anim *anim, int position)
{
...
static ImBuf *ffmpeg_fetchibuf(ImBuf_Anim *anim, int position, IMB_Timecode_Type tc)
{
  if (anim == nullptr) {
    return nullptr;
  }
  ...
  ImBuf_AnimIndex *tc_index = IMB_anim_open_index(anim, tc);
 ...
 */
static bool anim_getnew(ImBuf_Anim *anim)
{
                         ...
                         IMB_Proxy_Size preview_size)
{
  ...
  if (preview_size == IMB_PROXY_NONE) {
    ...
    ImBuf_Anim *proxy = IMB_anim_open_proxy(anim, preview_size);
