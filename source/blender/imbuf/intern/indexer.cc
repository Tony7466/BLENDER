...
ImBuf_AnimIndex *IMB_indexer_open(const char *filepath)
{
  char header[13];
  ImBuf_AnimIndex *idx;
  ...
  idx = MEM_cnew<ImBuf_AnimIndex>("ImBuf_AnimIndex");
...
uint64_t IMB_indexer_get_seek_pos(ImBuf_AnimIndex *idx, int frame_index)
{
...
uint64_t IMB_indexer_get_seek_pos_pts(ImBuf_AnimIndex *idx, int frame_index)
{
  if (frame_index < 0) {
    frame_index = 0;
  }
...
uint64_t IMB_indexer_get_seek_pos_dts(ImBuf_AnimIndex *idx, int frame_index)
{
  if (frame_index < 0) {
    frame_index = 0;
  }
...
int IMB_indexer_get_frame_index(ImBuf_AnimIndex *idx, int frameno)
{
...
uint64_t IMB_indexer_get_pts(ImBuf_AnimIndex *idx, int frame_index)
{
  if (frame_index < 0) {
    frame_index = 0;
  }
...
int IMB_indexer_get_duration(ImBuf_AnimIndex *idx)
{
...
int IMB_indexer_can_scan(ImBuf_AnimIndex *idx, int old_frame_index, int new_frame_index)
{
...
void IMB_indexer_close(ImBuf_AnimIndex *idx)
{
  MEM_freeN(idx->entries);
  MEM_freeN(idx);
}
...
static void get_index_dir(ImBuf_Anim *anim, char *index_dir, size_t index_dir_maxncpy)
{
  if (!anim->index_dir[0]) {
...
static bool get_proxy_filepath(ImBuf_Anim *anim,
...
static void get_tc_filepath(ImBuf_Anim *anim, IMB_Timecode_Type tc, char *filepath)
{
  char index_dir[FILE_MAXDIR];
...
static proxy_output_ctx *alloc_proxy_output_ffmpeg(
    ImBuf_Anim *anim, AVStream *st, IMB_Proxy_Size proxy_size, int width, int height, int quality)
{
...
static IndexBuildContext *index_ffmpeg_create_context(ImBuf_Anim *anim,
                                                      int tcs_in_use,
                                                      int proxy_sizes_in_use,
                                                      int quality,
                                                      bool build_only_on_bad_performance)
...
static AviMovie *alloc_proxy_output_avi(
    ImBuf_Anim *anim, const char *filepath, int width, int height, int quality)
{
  int x, y;
  AviFormat format;
  double framerate;
...
static IndexBuildContext *index_fallback_create_context(ImBuf_Anim *anim,
                                                        int /*tcs_in_use*/,
                                                        int proxy_sizes_in_use,
                                                        int quality)
{
...
void IMB_free_indices(ImBuf_Anim *anim)
{
  int i;
...
ImBuf_Anim *IMB_anim_open_proxy(ImBuf_Anim *anim, IMB_Proxy_Size preview_size)
{
  char filepath[FILE_MAX];
  int i = IMB_proxy_size_to_array_index(preview_size);
...
ImBuf_AnimIndex *IMB_anim_open_index(ImBuf_Anim *anim, IMB_Timecode_Type tc)
{
  char filepath[FILE_MAX];
  int i = IMB_timecode_to_array_index(tc);
...
{
  ImBuf_AnimIndex *idx = IMB_anim_open_index(anim, tc);
