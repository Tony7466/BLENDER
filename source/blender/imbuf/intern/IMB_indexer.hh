...
struct ImBuf_AnimIndex *IMB_indexer_open(const char *name);
uint64_t IMB_indexer_get_seek_pos(struct ImBuf_AnimIndex *idx, int frame_index);
uint64_t IMB_indexer_get_seek_pos_pts(struct ImBuf_AnimIndex *idx, int frame_index);
uint64_t IMB_indexer_get_seek_pos_dts(struct ImBuf_AnimIndex *idx, int frame_index);

int IMB_indexer_get_frame_index(struct ImBuf_AnimIndex *idx, int frameno);
uint64_t IMB_indexer_get_pts(struct ImBuf_AnimIndex *idx, int frame_index);
int IMB_indexer_get_duration(struct ImBuf_AnimIndex *idx);

int IMB_indexer_can_scan(struct ImBuf_AnimIndex *idx, int old_frame_index, int new_frame_index);

void IMB_indexer_close(struct ImBuf_AnimIndex *idx);

void IMB_free_indices(struct ImBuf_Anim *anim);

struct ImBuf_Anim *IMB_anim_open_proxy(struct ImBuf_Anim *anim, IMB_Proxy_Size preview_size);
struct ImBuf_AnimIndex *IMB_anim_open_index(struct ImBuf_Anim *anim, IMB_Timecode_Type tc);
