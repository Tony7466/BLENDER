...
static void rna_def_light_shadow(StructRNA *srna, bool sun)
{
  ...
  if (sun) {
    ...
    RNA_def_property_ui_text(
        ...
        "Minimum size of a shadow map pixel. "
        "Higher values will reduce the cost of the shadow map in close-up regions. "
        "A value of 0 will always use the maximum resolution available");
    RNA_def_property_override_flag(prop, PROPOVERRIDE_OVERRIDABLE_LIBRARY);
    RNA_def_property_update(prop, 0, "rna_Light_update");
  }
  else {
