...
static int rna_ConsoleLine_cursor_get(PointerRNA *ptr)
{
  const ConsoleLine *ci = (ConsoleLine *)ptr->data;
  return BLI_str_utf8_offset_to_index(ci->line, ci->len, ci->cursor);
}

static void rna_ConsoleLine_cursor_set(PointerRNA *ptr, int index)
{
  ConsoleLine *ci = (ConsoleLine *)ptr->data;
...
static void rna_def_console_line(BlenderRNA *brna)
{
  ...
  RNA_def_property_int_funcs(
      prop, "rna_ConsoleLine_cursor_get", "rna_ConsoleLine_cursor_set", nullptr);
  RNA_def_property_update(prop, NC_SPACE | ND_SPACE_CONSOLE, nullptr);
