...
static void rna_MathFormulaItem_update(Main *bmain, Scene * /*scene*/, PointerRNA *ptr)
{
  bNodeTree *ntree = reinterpret_cast<bNodeTree *>(ptr->owner_id);
  NodeMathFormulaItem *item = static_cast<NodeMathFormulaItem *>(ptr->data);
  bNode *node = NOD_math_formula_find_node_by_item(ntree, item);

  BKE_ntree_update_tag_node_property(ntree, node);
  ED_node_tree_propagate_change(nullptr, bmain, ntree);
}

static void rna_MathFormulaItem_name_set(PointerRNA *ptr, const char *value)
{
  bNodeTree *ntree = reinterpret_cast<bNodeTree *>(ptr->owner_id);
  NodeMathFormulaItem *item = static_cast<NodeMathFormulaItem *>(ptr->data);
  bNode *node = NOD_math_formula_find_node_by_item(ntree, item);
  NOD_math_formula_rename_item(node, item, value);
}

static NodeMathFormulaItem *rna_NodeMathFormula_items_new(
    ID *id, bNode *node, Main *bmain, ReportList *reports, int socket_type, const char *name)
{
  NodeMathFormula *sim = static_cast<NodeMathFormula *>(node->storage);

  NodeMathFormulaItem *item = NOD_math_formula_add_item(sim, (short)socket_type, name);
...
static void rna_NodeMathFormula_items_remove(
    ID *id, bNode *node, Main *bmain, ReportList *reports, NodeMathFormulaItem *item)
{
  NodeMathFormula *sim = static_cast<NodeMathFormula *>(node->storage);

  if (!NOD_math_formula_contains_item(sim, item)) {
    BKE_reportf(reports, RPT_ERROR, "Unable to locate item '%s' in node", item->name);
  }
  else {
    NOD_math_formula_remove_item(sim, item);
...
static void rna_NodeMathFormula_items_clear(ID *id, bNode *node, Main *bmain)
{
  NodeMathFormula *sim = static_cast<NodeMathFormula *>(node->storage);
  NOD_math_formula_clear_items(sim);
...
static void rna_NodeMathFormula_items_move(
    ID *id, bNode *node, Main *bmain, int from_index, int to_index)
{
  NodeMathFormula *sim = static_cast<NodeMathFormula *>(node->storage);
  ...
  NOD_math_formula_move_item(sim, from_index, to_index);
...
static PointerRNA rna_NodeMathFormula_active_item_get(PointerRNA *ptr)
{
  bNode *node = static_cast<bNode *>(ptr->data);
  NodeMathFormula *sim = static_cast<NodeMathFormula *>(node->storage);
  NodeMathFormulaItem *item = NOD_math_formula_get_active_item(sim);
  PointerRNA r_ptr;
  RNA_pointer_create(ptr->owner_id, &RNA_NodeMathFormulaItem, item, &r_ptr);
  return r_ptr;
}

static void rna_NodeMathFormula_active_item_set(PointerRNA *ptr,
                                                PointerRNA value,
                                                ReportList * /*reports*/)
{
  bNode *node = static_cast<bNode *>(ptr->data);
  NodeMathFormula *sim = static_cast<NodeMathFormula *>(node->storage);
  NOD_math_formula_set_active_item(sim, static_cast<NodeMathFormulaItem *>(value.data));
}

static void rna_NodeMathFormula_item_color_get(PointerRNA *ptr, float *values)
{
  NodeMathFormulaItem *item = static_cast<NodeMathFormulaItem *>(ptr->data);
...
static void rna_def_math_formula_item(BlenderRNA *brna)
{
  PropertyRNA *prop;
  ...
  StructRNA *srna = RNA_def_struct(brna, "NodeMathFormulaItem", nullptr);
  RNA_def_struct_ui_text(srna, "Math formula Input Item", "");

  prop = RNA_def_property(srna, "name", PROP_STRING, PROP_NONE);
  RNA_def_property_string_funcs(prop, nullptr, nullptr, "rna_MathFormulaItem_name_set");
  RNA_def_property_ui_text(prop, "Name", "");
  RNA_def_struct_name_property(srna, prop);
  RNA_def_property_update(prop, NC_NODE | NA_EDITED, "rna_MathFormulaItem_update");
  ...
  RNA_def_property_array(prop, 4);
  RNA_def_property_float_funcs(prop, "rna_NodeMathFormula_item_color_get", nullptr, nullptr);
  RNA_def_property_clear_flag(prop, PROP_EDITABLE);
  RNA_def_property_ui_text(
      prop, "Color", "Color of the corresponding socket type in the node editor");
}

static void def_cmp_math_formula_items(BlenderRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *parm;
  FunctionRNA *func;

  srna = RNA_def_struct(brna, "NodeMathFormulaItems", nullptr);
  RNA_def_struct_sdna(srna, "bNode");
  RNA_def_struct_ui_text(srna, "Items", "Collection of formula input items");

  func = RNA_def_function(srna, "new", "rna_NodeMathFormula_items_new");
  RNA_def_function_ui_description(func, "Add a item to this formula node");
  RNA_def_function_flag(func, FUNC_USE_SELF_ID | FUNC_USE_MAIN | FUNC_USE_REPORTS);
  ...
  RNA_def_parameter_flags(parm, PropertyFlag(0), PARM_REQUIRED);
  /* return value */
  parm = RNA_def_pointer(func, "item", "NodeMathFormulaItem", "Item", "New input item");
  RNA_def_function_return(func, parm);

  func = RNA_def_function(srna, "remove", "rna_NodeMathFormula_items_remove");
  RNA_def_function_ui_description(func, "Remove an item from this formula node");
  RNA_def_function_flag(func, FUNC_USE_SELF_ID | FUNC_USE_MAIN | FUNC_USE_REPORTS);
  parm = RNA_def_pointer(func, "item", "NodeMathFormulaItem", "Item", "The item to remove");
  RNA_def_parameter_flags(parm, PROP_NEVER_NULL, PARM_REQUIRED);

  func = RNA_def_function(srna, "clear", "rna_NodeMathFormula_items_clear");
  RNA_def_function_ui_description(func, "Remove all items from this formula node");
  RNA_def_function_flag(func, FUNC_USE_SELF_ID | FUNC_USE_MAIN);

  func = RNA_def_function(srna, "move", "rna_NodeMathFormula_items_move");
...
static void def_cmp_math_formula(StructRNA *srna)
{
  static const EnumPropertyItem node_socket_data_type_items[] = {
      {SOCK_FLOAT, "FLOAT", 0, "Float", ""},
      {SOCK_INT, "INT", 0, "Integer", ""},
  ...
  RNA_def_property_struct_type(prop, "NodeMathFormulaItem");
  RNA_def_property_ui_text(prop, "Items", "");
  RNA_def_property_srna(prop, "NodeMathFormulaItems");
  ...
  RNA_def_property_struct_type(prop, "NodeMathFormulaItem");
  RNA_def_property_pointer_funcs(prop,
                                 "rna_NodeMathFormula_active_item_get",
                                 "rna_NodeMathFormula_active_item_set",
                                 nullptr,
                                 nullptr);
  RNA_def_property_flag(prop, PROP_EDITABLE);
  RNA_def_property_ui_text(prop, "Active Item Index", "Index of the active item");
...
void RNA_def_nodetree(BlenderRNA *brna)
{
  ...
  rna_def_math_formula_item(brna);
...
#  include "../../nodes/NOD_static_types.h"
  ...
  def_cmp_math_formula_items(brna);
