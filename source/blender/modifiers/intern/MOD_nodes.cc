                                     ...
                                     bool *r_real_time)
{
  ...
  for (const bNode *node : tree.group_nodes()) {
    if (const bNodeTree *sub_tree = reinterpret_cast<const bNodeTree *>(node->id)) {
      check_tree_for_time_node(*sub_tree, checked_groups, r_scene_time, r_real_time);
    }
    /* Early exit. */
...
static void dependsOnTime(struct Scene * /*scene*/,
                          ModifierData *md,
                          bool *r_scene_time,
                          bool *r_real_time)
{
  ...
  Set<const bNodeTree *> checked_groups;
  check_tree_for_time_node(*tree, checked_groups, r_scene_time, r_real_time);
}
    ...
    nodes::GeoNodesModifierData &geo_nodes_modifier_data)
{
  ...
  const SubFrame current_frame = DEG_get_ctime_ex(ctx.depsgraph, time_source_type);
  ...
  if (DEG_is_active(ctx.depsgraph)) {
...

    {
      ...
      else if (time_source_type == eTimeSourceType::DEG_TIME_SOURCE_REALTIME) {
        /* Realtime clock simulation runs indefinitely, the cache just needs the last state. */
    ...
    if (nmd_orig.simulation_cache->cache_state() != bke::sim::CacheState::Baked) {
      if (sim_states.current == nullptr) {
        if (is_start_frame || !nmd_orig.simulation_cache->has_states()) {
          ...
          if (!is_start_frame) {
            /* When starting a new simulation at another frame than the start frame, it can't
             * match what would be baked, so invalidate it immediately. */
