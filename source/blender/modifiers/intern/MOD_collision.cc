  ...
  if (mesh) {
    ...
    if (collmd->time_xnew == -1000) { /* first time */
      ...
      collmd->x = static_cast<float(*)[3]>(
          MEM_dupallocN(BKE_mesh_vert_positions(mesh))); /* frame start position */
...

      {
        ...
        BKE_mesh_runtime_verttri_from_looptri(
            tri, BKE_mesh_corner_verts(mesh), looptri, collmd->tri_num);
        collmd->tri = tri;
      }
      ...
      memcpy(collmd->xnew, BKE_mesh_vert_positions(mesh), mvert_num * sizeof(float[3]));
