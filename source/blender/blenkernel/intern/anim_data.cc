...
void BKE_animdata_transfer_by_basepath(Main *bmain, ID *srcID, ID *dstID, ListBase *basepaths)
{
  ...
  DEG_id_tag_update(&srcAdt->action->id, ID_RECALC_EVALUATED_COPY);
}
                                   ...
                                   bool verify_paths)
{
  ...
  if (adt->tmpact) {
    ...
    {
      DEG_id_tag_update(&adt->tmpact->id, ID_RECALC_EVALUATED_COPY);
    }
  }
  ...
  /* Tag owner ID if it */
  if (is_self_changed) {
    DEG_id_tag_update(owner_id, ID_RECALC_EVALUATED_COPY);
  }
  /* free the temp names */
  MEM_freeN(oldN);
  MEM_freeN(newN);
