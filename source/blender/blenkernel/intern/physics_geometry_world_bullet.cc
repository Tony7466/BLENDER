...
// Both Bullet and Jolt implement the attribute providers, creating a conflict.
// For now just disable Bullet implementation entirely.
#if 0
...
static float4x4 get_constraint_frame1(const btTypedConstraint &constraint)
{
...
static void set_constraint_frame1(btTypedConstraint &constraint, float4x4 value)
{
  using ConstraintType = PhysicsConstraintType;
...
static float4x4 get_constraint_frame2(const btTypedConstraint &constraint)
{
  using ConstraintType = PhysicsConstraintType;
...
static void set_constraint_frame2(btTypedConstraint &constraint, float4x4 value)
{
  using ConstraintType = PhysicsConstraintType;
...
static bool validate_bullet_constraint_type(const PhysicsConstraintType type,
...
/* -------------------------------------------------------------------- */
/** \name Bullet Physics World Data
 * \{ */

BulletPhysicsWorldData::BulletPhysicsWorldData()
{
  this->create_world();
}

BulletPhysicsWorldData::BulletPhysicsWorldData(int body_num, int constraint_num)
{
  this->create_world();
...
void BulletPhysicsWorldData::resize(const int body_num, const int constraint_num)
{
...
void BulletPhysicsWorldData::resize(const int body_num,
...
void BulletPhysicsWorldData::move(const IndexMask &src_body_mask,
                                  const IndexMask &src_constraint_mask,
                                  int dst_body_offset,
...
void BulletPhysicsWorldData::tag_bodies_in_world() const
{
  bodies_in_world_cache_.tag_dirty();
}

void BulletPhysicsWorldData::tag_constraints_in_world() const
{
  constraints_in_world_cache_.tag_dirty();
}

void BulletPhysicsWorldData::ensure_body_and_constraint_indices() const
{
...
void BulletPhysicsWorldData::ensure_bodies_and_constraints_in_world()
{
...
void BulletPhysicsWorldData::create_constraints(const IndexMask &selection,
...
void BulletPhysicsWorldData::set_disable_collision(const IndexMask &selection,
...
void BulletPhysicsWorldData::set_overlap_filter(OverlapFilterFn fn)
{
...
void BulletPhysicsWorldData::clear_overlap_filter()
{
  if (overlap_filter_) {
...
float3 BulletPhysicsWorldData::gravity() const
{
  return to_blender(world_->getGravity());
}

void BulletPhysicsWorldData::set_gravity(const float3 &gravity)
{
  world_->setGravity(to_bullet(gravity));
}

void BulletPhysicsWorldData::set_solver_iterations(const int num_solver_iterations)
{
  btContactSolverInfo &info = world_->getSolverInfo();
  info.m_numIterations = num_solver_iterations;
}

void BulletPhysicsWorldData::set_split_impulse(const bool split_impulse)
{
  btContactSolverInfo &info = world_->getSolverInfo();
...
void BulletPhysicsWorldData::step_simulation(float delta_time)
{
...
void BulletPhysicsWorldData::set_body_shapes(const IndexMask &selection,
...
void BulletPhysicsWorldData::set_body_static(const IndexMask &selection,
                                             const Span<bool> is_static)
{
...
void BulletPhysicsWorldData::set_body_mass(const IndexMask &selection, const Span<float> masses)
{
  bool removed_body = false;
  selection.foreach_index([&](const int index) {
    btRigidBody *body = this->rigid_bodies_[index];
...
void BulletPhysicsWorldData::apply_force(const IndexMask &selection,
...
void BulletPhysicsWorldData::apply_torque(const IndexMask &selection,
                                          const VArray<float3> &torques)
{
  selection.foreach_index([&](const int index) {
...
void BulletPhysicsWorldData::apply_impulse(const IndexMask &selection,
...
void BulletPhysicsWorldData::apply_angular_impulse(const IndexMask &selection,
...
void BulletPhysicsWorldData::clear_forces(const IndexMask &selection)
{
  selection.foreach_index([&](const int index) { rigid_bodies_[index]->clearForces(); });
}

Span<btRigidBody *> BulletPhysicsWorldData::bodies() const
{
  return rigid_bodies_;
}

Span<btTypedConstraint *> BulletPhysicsWorldData::constraints() const
{
  return constraints_;
}

const btDynamicsWorld &BulletPhysicsWorldData::world() const
{
  return *world_;
}

void BulletPhysicsWorldData::create_world()
...
void BulletPhysicsWorldData::destroy_world()
{
...
AttributeAccessor BulletPhysicsWorldData::attributes() const
{
  return AttributeAccessor(this, bke::get_accessor_functions_ref());
}

MutableAttributeAccessor BulletPhysicsWorldData::attributes_for_write()
{
...
bool BulletPhysicsWorldData::validate(const AttributeAccessor attributes,
                                      const Span<CollisionShapePtr> shapes)
{
  bool ok = true;

  this->ensure_body_and_constraint_indices();
  this->ensure_bodies_and_constraints_in_world();

  // AttributeAccessor cached_attributes = custom_data_attributes();
  const Span<btRigidBody *> rigid_bodies = this->bodies();
  const Span<btTypedConstraint *> constraints = this->constraints();
  const btCollisionObjectArray &bt_collision_objects = this->world().getCollisionObjectArray();
  ...
  const VArray<int> cached_constraint_types = *attributes.lookup<int>(
      physics_attribute_name(ConstraintAttribute::constraint_type), AttrDomain::Edge);
  const VArray<int> cached_constraint_body1 = *attributes.lookup<int>(
      physics_attribute_name(ConstraintAttribute::constraint_body1), AttrDomain::Edge);
  const VArray<int> cached_constraint_body2 = *attributes.lookup<int>(
      physics_attribute_name(ConstraintAttribute::constraint_body2), AttrDomain::Edge);
  const VArray<bool> cached_constraint_disable_collision = *attributes.lookup<bool>(
  ...
  for (const int i : constraints.index_range()) {
    ...
    if (&constraint->getRigidBodyA() != &constraint->getRigidBodyB()) {
      ...
      for (const int i : IndexRange(this->world().getNumConstraints())) {
        if (this->world().getConstraint(i) == constraint) {
  ...
  const VArray<int> cached_body_shapes = *attributes.lookup<int>(
      physics_attribute_name(BodyAttribute::collision_shape), AttrDomain::Point);
  const IndexRange shape_range = shapes.index_range();
                                                      ...
                                                      PhysicsWorldData &world_data)
{
  ...
  switch (attribute) {
    ...
    case BodyAttribute::is_active:
      // TODO This should take allow_sleep into account, but will need a state attribute.
