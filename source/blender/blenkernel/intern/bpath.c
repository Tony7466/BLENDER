                                             ...
                                             const char *path_src)
{
  ...
  if (BLI_path_is_rel(path_dst)) {
    ...
    BKE_reportf(data->reports, RPT_WARNING, "Path '%s' cannot be made relative for %s '%s'", path_src, type_name, id_name);
    data->count_failed++;
  }
  return true;
}
                                             ...
                                             const char *path_src)
{
  ...
  if (BLI_path_is_rel(path_dst) == false) {
    ...
    const char* type_name = BKE_idtype_get_info_from_id(bpath_data->owner_id)->name;
    const char* id_name = bpath_data->owner_id->name + 2;
    BKE_reportf(data->reports, RPT_WARNING, "Path '%s' cannot be made absolute for %s '%s'", path_src, type_name, id_name);
    data->count_failed++;
  }
  return true;
}
