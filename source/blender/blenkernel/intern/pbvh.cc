...
static void calc_mesh_intersect_data(const Span<int> corner_verts,
                                     ...
                                     const float3 &ray_normal,
                                     const int tri_index,
                                     const std::array<const float3 &, 3> co,
                                     float *depth,
...
static void calc_grids_intersect_data(const CCGKey &key,
                                      ...
                                      const short x,
                                      const short y,
                                      const std::array<const float3 &, 4> co,
                                      float *depth,
                                      PBVHVertRef *r_active_vertex,
    ...
    for (const int grid : grids) {
      ...
      for (const short y : IndexRange(grid_size - 1)) {
        for (const short x : IndexRange(grid_size - 1)) {
          ...
          const std::array<const float3 &, 4> co{CCG_grid_elem_co(key, elem, x, y + 1),
    ...
    for (const int i : grids.index_range()) {
      ...
      for (const short y : IndexRange(grid_size - 1)) {
        for (const short x : IndexRange(grid_size - 1)) {
          ...
          const std::array<const float3 &, 4> co{grid_positions[(y + 1) * grid_size + x],
