  ...
  switch (from) {
    ...
    case PART_FROM_VOLUME: {
      MFace *mfaces = static_cast<MFace *>(
          CustomData_get_layer_for_write(&mesh->fdata, CD_MFACE, mesh->totface_legacy));
                                 ...
                                 LinkNode **poly_nodes)
{
  ...
  mtessface_final = static_cast<MFace *>(CustomData_get_layer_for_write(
      &mesh_final->fdata, CD_MFACE, mesh_final->totface_legacy));
  osface_final = static_cast<OrigSpaceFace *>(CustomData_get_layer_for_write(
      &mesh_final->fdata, CD_ORIGSPACE, mesh_final->totface_legacy));
    ...
    if (from == PART_FROM_VERT) {
      ...
      *mapindex = i;
      ...
      OrigSpaceFace *osface = static_cast<OrigSpaceFace *>(
          CustomData_get_layer_for_write(&mesh->fdata, CD_ORIGSPACE, mesh->totface_legacy));
                         ...
                         float orco[3])
{
  ...
  if (from == PART_FROM_VERT) {
    ...
    mtface = static_cast<MTFace *>(CustomData_get_layer_for_write(
        &mesh_final->fdata, CD_MTFACE, mesh_final->totface_legacy));
...
static void psys_face_mat(Object *ob, Mesh *mesh, ParticleData *pa, float mat[4][4], int orco)
{
  ...
  const OrigSpaceFace *osface = static_cast<const OrigSpaceFace *>(
      CustomData_get_for_write(&mesh->fdata, i, CD_ORIGSPACE, mesh->totface_legacy));
                           ...
                           bool from_vert)
{
  ...
  tf = static_cast<const MTFace *>(
      CustomData_get_layer_named(&mesh->fdata, CD_MTFACE, name));
  if (tf == nullptr) {
        ...
        if (uv_idx >= 0) {
          ...
          if (mtface != nullptr) {
            const MFace *mface = static_cast<const MFace *>(
                CustomData_get_for_write(&psmd->mesh_final->fdata,
                                         cpa->num,
                                         CD_MFACE,
    ...
    if (!ELEM(num, DMCACHE_NOTFOUND, DMCACHE_ISCHILD)) {
      ...
      if (uv_idx >= 0) {
        ...
        const MFace *mface = static_cast<const MFace *>(CustomData_get_for_write(
            &psmd->mesh_final->fdata, num, CD_MFACE, psmd->mesh_final->totface_legacy));
        mtface += num;
