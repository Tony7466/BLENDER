...
void blo_join_main(ListBase *mainlist)
{
  ...
  while ((tojoin = mainl->next)) {
    BLI_assert(((tojoin->curlib->runtime.tag & LIBRARY_IS_MINIMAL_ASSET_EDIT_FILE) != 0) ==
...
void blo_split_main(ListBase *mainlist, Main *main)
{
       ...
       lib = static_cast<Library *>(lib->id.next), i++)
  {
    ...
    libmain->is_asset_edit_file = (lib->runtime.tag & LIBRARY_IS_MINIMAL_ASSET_EDIT_FILE) != 0;
...
static void read_file_version(FileData *fd, Main *main)
{
  ...
  for (bhead = blo_bhead_first(fd); bhead; bhead = blo_bhead_next(fd, bhead)) {
    if (bhead->code == BLO_CODE_GLOB) {
      ...
      if (fg) {
        ...
        main->is_asset_edit_file = (fg->fileflags & G_FILE_MINIMAL_ASSET_EDIT_FILE) != 0;
        MEM_freeN(fg);
      }
  ...
  if (main->curlib) {
    ...
    if (main->is_asset_edit_file) {
      main->curlib->runtime.tag |= LIBRARY_IS_MINIMAL_ASSET_EDIT_FILE;
    }
  }
}
...
static BHead *read_global(BlendFileData *bfd, FileData *fd, BHead *bhead)
{
  ...
  bfd->main->is_asset_edit_file = (fg->fileflags & G_FILE_MINIMAL_ASSET_EDIT_FILE) != 0;
