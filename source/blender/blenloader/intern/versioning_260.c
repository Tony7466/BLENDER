            ...
            for (md = ob->modifiers.first; md; md = md->next) {
              if (md->type == eModifierType_Fluid) {
                ...
                if ((fmd->type & MOD_FLUID_TYPE_DOMAIN) && fmd->domain) {
                  int maxres = max_iii(
                      fmd->domain->res[0], fmd->domain->res[1], fmd->domain->res[2]);
        ...
        if (!MAIN_VERSION_ATLEAST(bmain, 263, 19)) {
          ...
          for (ima = bmain->images.first; ima; ima = ima->id.next) {
            if (ima->source == IMA_SRC_VIEWER) {
               ...
               * conversion would happen on display and render there's no clear way to check
               * whether color management is enabled or not in render engine so set all images to
               * raw if there's at least one scene with color management disabled this would still
               * behave incorrect in cases when color management was used for only some of scenes,
               * but such a setup is crazy anyway and think it's fair enough to break compatibility
               * in that cases.
               */
