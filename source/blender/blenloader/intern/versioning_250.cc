...
#  include "winsock2.h"
#  include <iostream> /* for open close read */
#endif
...
{
  ARegion *region = MEM_new<ARegion>("area region from do_versions");
...
static void area_add_window_regions(ScrArea *area, SpaceLink *sl, ListBase *lb)
{
  ...
  if (sl) {
    ...
    switch (sl->spacetype) {
      case SPACE_GRAPH:
        region = MEM_new<ARegion>("area region from do_versions");
        BLI_addtail(lb, region);
      ...
      case SPACE_NODE:
        region = MEM_new<ARegion>("nodetree area for node");
        BLI_addtail(lb, region);
        region->regiontype = RGN_TYPE_UI;
        region->alignment = RGN_ALIGN_LEFT;
        region->v2d.scroll = (V2D_SCROLL_RIGHT | V2D_SCROLL_BOTTOM);
      ...
      case SPACE_FILE:
        ...
        region = MEM_new<ARegion>("ui area for file");
        BLI_addtail(lb, region);
        region->regiontype = RGN_TYPE_UI;
        region->alignment = RGN_ALIGN_TOP;
        break;
      case SPACE_SEQ: {
        ...
        region = MEM_new<ARegion>("preview area for sequencer");
      ...
      case SPACE_VIEW3D:
        /* toolbar */
        region = MEM_new<ARegion>("toolbar for view3d");
        ...
        /* tool properties */
        region = MEM_new<ARegion>("tool properties for view3d");
        ...
        /* buttons/list view */
        region = MEM_new<ARegion>("buttons for view3d");
...
static void do_versions_windowmanager_2_50(bScreen *screen)
{
  ...
  LISTBASE_FOREACH (ScrArea *, area, &screen->areabase) {
    ...
    area_add_window_regions(area, static_cast<SpaceLink *>(area->spacedata.first), &area->regionbase);
...
static void do_versions_socket_default_value_259(bNodeSocket *sock)
{
  ...
  switch (sock->type) {
    case SOCK_FLOAT:
      sock->default_value = valfloat = MEM_new<bNodeSocketValueFloat>("default socket value");
      ...
      break;
    case SOCK_VECTOR:
      sock->default_value = valvector = MEM_new<bNodeSocketValueVector>("default socket value");
      ...
      break;
    case SOCK_RGBA:
      sock->default_value = valrgba = MEM_new<bNodeSocketValueRGBA>("default socket value");
...
void blo_do_versions_250(FileData *fd, Library */*lib*/, Main *bmain)
{
    ...
    LISTBASE_FOREACH (Scene *, sce, &bmain->scenes) {
      ...
      if (sce->r.mode & R_PANORAMA) {
        LISTBASE_FOREACH (Base *, base, &sce->base) {
          Object *ob = static_cast<Object *>(blo_do_versions_newlibadr(fd, &sce->id, ID_IS_LINKED(sce), base->object));
          if (ob->type == OB_CAMERA && !ob->id.lib) {
            Camera *cam = static_cast<Camera *>(blo_do_versions_newlibadr(fd, &ob->id, ID_IS_LINKED(ob), ob->data));
  ...
  if (!MAIN_VERSION_FILE_ATLEAST(bmain, 250, 1)) {
    ...
    LISTBASE_FOREACH (Object *, ob, &bmain->objects) {
      ...
      if (ob->totcol && ob->matbits == nullptr) {
        ...
        ob->matbits = MEM_cnew_array<char>(ob->totcol, "ob->matbits");
  ...
  if (!MAIN_VERSION_FILE_ATLEAST(bmain, 250, 7)) {
    ...
    LISTBASE_FOREACH (Mesh *, me, &bmain->meshes) {
      if ((key = static_cast<Key *>(blo_do_versions_newlibadr(fd, &me->id, ID_IS_LINKED(me), me->key))) && key->refkey)
      {
    ...
    LISTBASE_FOREACH (Lattice *, lt, &bmain->lattices) {
      if ((key = static_cast<Key *>(blo_do_versions_newlibadr(fd, &lt->id, ID_IS_LINKED(lt), lt->key))) && key->refkey)
      {
        data = static_cast<const float *>(key->refkey->data);
    ...
    LISTBASE_FOREACH (Curve *, cu, &bmain->curves) {
      if ((key = static_cast<Key *>(blo_do_versions_newlibadr(fd, &cu->id, ID_IS_LINKED(cu), cu->key))) && key->refkey)
      {
        data = static_cast<const float *>(key->refkey->data);
        ...
        LISTBASE_FOREACH (ScrArea *, area, &screen->areabase) {
          LISTBASE_FOREACH (SpaceLink *, sl, &area->spacedata) {
            if (sl->spacetype == SPACE_SEQ) {
              ...
              ARegion *region = MEM_new<ARegion>("preview area for sequencer");
  ...
  if (!MAIN_VERSION_FILE_ATLEAST(bmain, 250, 12)) {
    ...
    LISTBASE_FOREACH (Object *, ob, &bmain->objects) {
       ...
       */
      if (ob->pose && ob->data) {
        bArmature *arm = static_cast<bArmature *>(blo_do_versions_newlibadr(fd, &ob->id, ID_IS_LINKED(ob), ob->data));
