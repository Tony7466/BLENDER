...
static void do_versions_nodetree_convert_angle(bNodeTree *ntree)
{
  LISTBASE_FOREACH (bNode *, node, &ntree->nodes) {
    if (node->type == CMP_NODE_ROTATE) {
      ...
      bNodeSocket *sock =
          static_cast<bNodeSocket *>(static_cast<bNodeSocket *>(node->inputs.first))->next;
      sock->default_value_typed<bNodeSocketValueFloat>()->value = DEG2RADF(
          sock->default_value_typed<bNodeSocketValueFloat>()->value);
    }
...
static void do_versions_nodetree_multi_file_output_format_2_62_1(Scene *sce, bNodeTree *ntree)
{
  LISTBASE_FOREACH (bNode *, node, &ntree->nodes) {
    if (node->type == CMP_NODE_OUTPUT_FILE) {
      ...
      NodeImageMultiFile *nimf = static_cast<NodeImageMultiFile *>(
...
static void do_versions_nodetree_image_layer_2_64_5(bNodeTree *ntree)
{
  LISTBASE_FOREACH (bNode *, node, &ntree->nodes) {
    if (node->type == CMP_NODE_IMAGE) {
      LISTBASE_FOREACH (bNodeSocket *, sock, &node->outputs) {
        NodeImageLayer *output = static_cast<NodeImageLayer *>(
  ...
  LISTBASE_FOREACH (bNode *, node, &ntree->nodes) {
    if (node->type == NODE_FRAME) {
      ...
      if (node->storage == nullptr) {
        NodeFrame *data = static_cast<NodeFrame *>(
    ...
    if (ntree->type == NTREE_COMPOSIT) {
      LISTBASE_FOREACH (bNode *, node, &ntree->nodes) {
        if (node->type == CMP_NODE_DILATEERODE) {
          if (node->storage == nullptr) {
            NodeDilateErode *data = static_cast<NodeDilateErode *>(
                MEM_callocN(sizeof(NodeDilateErode), __func__));
