...
static bGPDpalettecolor *BKE_gpencil_palettecolor_addnew(bGPDpalette *palette, const char *name)
{
  ...
  /* allocate memory and add to end of list */
  palcolor = static_cast<bGPDpalettecolor *>(MEM_callocN(sizeof(bGPDpalettecolor), "bGPDpalettecolor"));
...
static void do_version_action_editor_properties_region(ListBase *regionbase)
{
  LISTBASE_FOREACH(ARegion *, region, regionbase) {
    ...
    if (region->regiontype == RGN_TYPE_WINDOW) {
      /* add new region here */
      ARegion *region_new = static_cast<ARegion *>(MEM_callocN(sizeof(ARegion), "buttons for action"));
...
static void do_versions_compositor_render_passes_storage(bNode *node)
{
  int pass_index = 0;
  const char *sockname;
  for (bNodeSocket *sock = static_cast<bNodeSocket *>(node->outputs.first); sock && pass_index < 31;
       sock = static_cast<bNodeSocket *>(sock->next), pass_index++)
  {
    if (sock->storage == nullptr) {
      NodeImageLayer *sockdata = static_cast<NodeImageLayer *>(MEM_callocN(sizeof(NodeImageLayer), "node image layer"));
...
static void do_version_bbone_easing_fcurve_fix(ID */*id*/,
                                               FCurve *fcu,
                                               void */*user_data*/)
{
...
{
  seq->stereo3d_format = static_cast<Stereo3dFormat *>(MEM_callocN(sizeof(Stereo3dFormat), "Stereo Display 3d Format"));
...
#undef BRUSH_RANDOM_ROTATION
        ...
        if (ntree->type == NTREE_COMPOSIT) {
          LISTBASE_FOREACH(bNode *, node, &ntree->nodes) {
            if (ELEM(node->type, CMP_NODE_PLANETRACKDEFORM)) {
              NodePlaneTrackDeformData *data = static_cast<NodePlaneTrackDeformData *>(node->storage);
              data->flag = 0;
  ...
  if (!MAIN_VERSION_ATLEAST(bmain, 274, 4)) {
    ...
    LISTBASE_FOREACH(Image *, ima, &bmain->images) {
      ima->stereo3d_format = static_cast<Stereo3dFormat *>(MEM_callocN(sizeof(Stereo3dFormat), "Image Stereo 3d Format"));

      if (ima->packedfile) {
        ImagePackedFile *imapf = static_cast<ImagePackedFile *>(MEM_mallocN(sizeof(ImagePackedFile), "Image Packed File"));
    ...
    LISTBASE_FOREACH(wmWindowManager *, wm, &bmain->wm) {
      LISTBASE_FOREACH(wmWindow *, win, &wm->windows) {
        win->stereo3d_format = static_cast<Stereo3dFormat *>(MEM_callocN(sizeof(Stereo3dFormat), "Stereo Display 3d Format"));
      }
    }
  }
