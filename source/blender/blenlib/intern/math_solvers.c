                        ...
                        float result[3])
{
  ...
  for (int i = 0; i == 0 || (i < max_iterations && fdeltav > epsilon); i++) {
    ...
    for (int j = 0; next_fdeltav > fdeltav && next_fdeltav > epsilon; j++) {
      if (j >= max_line_steps) {
        /* This means the iteration step was extremely big compared to the correct distance,
    ...
    if (func_correction) {
      switch (func_correction(userdata, x, step, x_next)) {
        case NEWTON3D_ABORT:
          return false;
        case NEWTON3D_CONTINUE:
          break;
        case NEWWON3D_MODIFIED:
          func_delta(userdata, x_next, fdelta);
          next_fdeltav = len_squared_v3(fdelta);
