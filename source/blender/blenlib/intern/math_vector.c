...
float angle_normalized_v3v3(const float v1[3], const float v2[3])
{
  ...
  if (dot_v3v3(v1, v2) >= 0.0f) {
    return 2.0f * saasinf(len_v3v3(v1, v2) / 2.0f);
  }

  float v2_n[3];
  negate_v3_v3(v2_n, v2);
  return (float)M_PI - 2.0f * saasinf(len_v3v3(v1, v2_n) / 2.0f);
}
...
float angle_normalized_v2v2(const float a[2], const float b[2])
{
  ...
  if (dot_v2v2(a, b) >= 0.0f) {
    return 2.0f * saasinf(len_v2v2(a, b) / 2.0f);
  }

  float v2_n[2];
  negate_v2_v2(v2_n, b);
  return (float)M_PI - 2.0f * saasinf(len_v2v2(a, v2_n) / 2.0f);
}
