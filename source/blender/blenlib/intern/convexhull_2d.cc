                                                      ...
                                                      int points_hull_num)
{
  ...
  for (int i = 0; i < points_hull_num; i++) {
    ...
    const float2 sincos = math::normalize_and_get_length(
        float2(points_hull[i_next]) - float2(points_hull[i]), dir_length);
    if (UNLIKELY(dir_length == 0.0f)) {
      continue;
    }
...
/* -------------------------------------------------------------------- */
/** \name Hull Angle Iteration AABB (Optimized)
 *
...
{
  BLI_assert(hstep.index_next != -1);
  while (hstep.index_next != hstep.index_max) {
    const int i_curr = hstep.index_next;
    const int i_next = (hstep.index_next + 1) % hiter.points_hull_num;
                                                   ...
                                                   const int points_hull_num)
{
  ...
  for (int i = 1; i < points_hull_num; i++) {
    ...
    for (int axis = 0; axis < 2; axis++) {
      if (range[axis][0] < p[axis]) {
        range[axis][0] = p[axis];
        hiter.axis[axis][0].index_next = i;
      }
      if (range[axis][1] > p[axis]) {
        range[axis][1] = p[axis];
        hiter.axis[axis][1].index_next = i;
      }
    }
  }

  /* Step backwards, compute the actual `sincos_canonical` because it's possible
   * an edge which is not *exactly* axis aligned normalizes to a value which is (when normalized).
  ...
  for (int axis = 0; axis < 2; axis++) {
    for (int i = 0; i < 2; i++) {
      int count = 0;
      const int i_orig = hiter.axis[axis][i].index_next;
      ...
      while ((i_prev = (i_curr + points_hull_num_minus_1) % points_hull_num) != i_orig) {
        ...
        i_curr = i_prev;
        hiter.axis[axis][i].index_next = i_curr;
        count++;
      }
    }
  }
  ...
  hiter.axis[0][0].index_max = hiter.axis[1][0].index_next; /* West to south. */
  hiter.axis[1][0].index_max = hiter.axis[0][1].index_next; /* South to east. */
  hiter.axis[0][1].index_max = hiter.axis[1][1].index_next; /* East to north. */
  hiter.axis[1][1].index_max = hiter.axis[0][0].index_next; /* North to west. */
  ...
  for (int axis = 0; axis < 2; axis++) {
    for (int i = 0; i < 2; i++) {
      hiter.axis[axis][i].angle.index = hiter.axis[axis][i].index_next;
...
static void convexhull_2d_angle_iter_step(HullAngleIter &hiter)
{
  ...
  /* Assert angle is properly ordered. */
#ifndef NDEBUG
  ...
  if (hiter.axis_ordered) {
    ...
    // printf("> %.12f %.12f (%d)\n", UNPACK2(hstep->angle.sincos_canonical), hstep->angle.index);
