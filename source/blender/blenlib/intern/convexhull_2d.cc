...
static float mul_v2_v2_cw_x(const float mat[2], const float vec[2])
{
  return (mat[0] * vec[0]) + (mat[1] * vec[1]);
}

static float mul_v2_v2_cw_y(const float mat[2], const float vec[2])
{
...
float BLI_convexhull_aabb_fit_hull_2d(const float (*points_hull)[2], int points_hull_num)
{
  ...
  float dvec_best[2]; /* Track the best angle as a unit vector, delaying `atan2`. */
  bool is_first = true;
  ...
  for (int i = 0, i_prev = points_hull_num - 1; i < points_hull_num; i_prev = i++) {
    ...
    sub_v2_v2v2(dvec, points_hull[i], points_hull[i_prev]);
