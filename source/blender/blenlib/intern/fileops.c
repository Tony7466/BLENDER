...
/**
 * In the simple case this is just `BLI_path_slash_rfind(dirname)` however it behaves differently
 * when there are redundant characters:
 *
 * `/test///dir///file`
 *             ^
 * `/test///dir//file/`
 *                   ^
 */
static char *path_parent_end(char *path, char path_len)
{
  char *path_end = path + path_len - 1;
  char *p = path_end;
  bool found = false;
  while (p >= path) {
  ...
  if (found) {
    while (p > path && BLI_path_slash_is_native_compat(*(p - 1))) {
      p--;
    }
...
static bool dir_create_recursive(char *dirname, int len)
{
  ...
  bool ret = true;
  char *dirname_parent_end = path_parent_end(dirname, len);
...
bool BLI_dir_create_recursive(const char *dirname)
{
  ...
  while (len && BLI_path_slash_is_native_compat(dirname_mut[len - 1])) {
    len--;
  }
  dirname_mut[len] = '\0';
