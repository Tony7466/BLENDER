...
{
  bNode *new_node = bke::nodeAddStaticNode(C, ntree, type);
...
{
  bNodeSocket *source_socket = bke::nodeFindSocket(source, SOCK_OUT, sock_out);
  ...
  bNodeSocket *dest_socket = bke::nodeFindSocket(dest, SOCK_IN, sock_in);
  ...
  bke::nodeAddLink(ntree, source, source_socket, dest, dest_socket);
}
...
{
  bNodeSocket *sock = bke::nodeFindSocket(node, SOCK_IN, identifier);
  if (!sock) {
    BKE_reportf(reports,
                RPT_ERROR,
                                                   ...
                                                   const pxr::UsdShadeShader &usd_shader) const
{
  ...
  bNodeSocket *emission_strength_sock = bke::nodeFindSocket(principled, SOCK_IN, "Emission Strength");
                                       ...
                                       bool is_color_corrected) const
{
  ...
  if (usd_input.HasConnectedSource()) {
    ...
    bNodeSocket *sock = bke::nodeFindSocket(dest_node, SOCK_IN, dest_socket_name);
    if (!sock) {
                                       ...
                                       NodePlacementContext *r_ctx)
{
  ...
  bNodeSocket *sock_scale = bke::nodeFindSocket(scale_bias.node, SOCK_IN, "Vector_001");
  bNodeSocket *sock_bias = bke::nodeFindSocket(scale_bias.node, SOCK_IN, "Vector_002");
                                              ...
                                              NodePlacementContext *r_ctx)
{
  ...
  bNodeSocket *sock_scale = bke::nodeFindSocket(adjust.node, SOCK_IN, "Vector_001");
  bNodeSocket *sock_bias = bke::nodeFindSocket(adjust.node, SOCK_IN, "Vector_002");
