...
{
  Expectation expect = {ASCII, 8, 0, 12, float3(-1, -1, -1), float3(1, 1, 1)};
  import_and_check("ASCII_wireframe_cube.ply", expect);
}

TEST_F(ply_import_test, PLYImportBunny)
{
  Expectation expect = {BINARY_LE,
                        1623,
                        1000,
                        0,
...
{
  Expectation expect = {BINARY_LE, 8, 0, 12, float3(-1, -1, -1), float3(1, 1, 1)};
