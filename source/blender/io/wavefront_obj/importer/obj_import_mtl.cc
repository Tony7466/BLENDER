...
{
  BLI_assert(r_node);
  bNodeSocket *socket{blender::bke::nodeFindSocket(r_node, SOCK_IN, socket_id)};
...
{
  bNode *node = blender::bke::nodeAddStaticNode(nullptr, ntree, type);
  node->locx = x;
  node->locy = y;
  return node;
}
...
{
  bNodeSocket *from_sock{blender::bke::nodeFindSocket(from_node, SOCK_OUT, from_node_id)};
  bNodeSocket *to_sock{blender::bke::nodeFindSocket(to_node, SOCK_IN, to_node_id)};
  BLI_assert(from_sock && to_sock);
  blender::bke::nodeAddLink(ntree, from_node, from_sock, to_node, to_sock);
}
