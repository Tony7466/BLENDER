                           ...
                           bke::MutableAttributeAccessor dst_attributes)
{
  ...
  const GroupedSpan<int> groups = gather_groups(src_to_dst_mapping, dst_domain_size, mix_groups_offsets, mix_groups_indices);

  src_attributes.for_all([&](const bke::AttributeIDRef &id, const bke::AttributeMetaData meta_data) {
    ...
    bke::attribute_math::convert_to_static_type(meta_data.data_type, [&](auto dymmu) {
      ...
      threading::parallel_for(groups.index_range(), 4098, [&](const IndexRange range) {
      ...
      threading::accumulated_task_sizes([&](const IndexRange range) { return groups.offsets[range].size(); }));
      values.finalize();
    });
...
static void ensure_min_face_sizes(const GroupedSpan<int> corner_verts, const int faces_num, MutableSpan<int> face_sizes, MutableSpan<bool> verts_to_dissolve_selection)
{
  IndexMaskMemory memory;
  const IndexMask invalid_faces = IndexMask::from_predicate(IndexMask(faces_num), GrainSize(4096), memory, [&](const int face_i) {
  ...
  invalid_faces.foreach_index_optimized<int>(GrainSize(4096), [&](const int face_i) {
    ...
    const int forced_face_vertices_num = std::count_if(front_corner_selection.begin(), front_corner_selection.end(), [&](const int v) { return verts_to_dissolve_selection[v]; });
                                              ...
                                              Array<int2> &r_new_edges)
{
  ...
  threading::parallel_for(IndexRange(src_edges.size()), 2048, [&](const IndexRange range) {
    for (const int edge_i : range) {
      old_to_new_edges_map[edge_i] = unique_edges.index_of(r_new_edges[old_to_new_edges_map[edge_i]]);
    }
  });
...
struct FaceKey {
  ...
  FaceKey(const int face_index, const GroupedSpan<int> faces) : face_i(face_index), first_corner_vert(faces[face_index][0]) {}
};
...
static Array<int> deduplicate_faces_for_verts(const GroupedSpan<int> dst_corner_verts, MutableSpan<int> old_to_new_faces_map, MutableSpan<int> old_to_new_corners_map)
{
  ...
  threading::parallel_for(dst_corner_verts.index_range(), 8192, [&](const IndexRange range) {
    for (const int face_i : range) {
      ...
      parallel_sort(corners.begin(), corners.end(), [&](const int corner_a, const int corner_b) {
        return unordered_verts[corner_a] < unordered_verts[corner_b];
      });
    }
  },
  threading::accumulated_task_sizes([&](const IndexRange range) { return dst_corner_verts.offsets[range].size(); }));
  ...
  VectorSet<FaceKey, DefaultProbingStrategy, FaceHash, FacesEquality> faces(FaceHash{}, FacesEquality{sorted_face_verts});
  ...
  threading::parallel_for(dst_corner_verts.index_range(), 8192, [&](const IndexRange range) {
    for (const int face_i : range) {
      ...
      std::transform(corners.begin(), corners.end(), mapped_corners.begin(), [&](const int ordered_i) {
                              ...
                              const bke::AnonymousAttributePropagationInfo &propagation_info)
{
  ...
  const GroupedSpan<int> vert_to_edge_map = bke::mesh::build_vert_to_edge_map(src_edges, src_mesh.verts_num, vert_to_edge_offsets, vert_to_edge_indices);

  IndexMaskMemory memory;
  const IndexMask boundary_verts_to_dissolve_mask = IndexMask::from_predicate(verts_mask, GrainSize(4096), memory, [&](const int vert_i) {
  ...
  threading::parallel_for(IndexRange(src_mesh.faces_num), 4096, [&](const IndexRange range) {
    for (const int face_i : range) {
      const IndexRange face = src_faces[face_i];
      const Span<int> corner_verts = src_corner_verts.slice(face);
      const int dissolved_verts_num = std::count_if(corner_verts.begin(), corner_verts.end(), [&](const int v) { return verts_to_dissolve_selection[v]; });
      new_faces_sizes[face_i] = face.size() - dissolved_verts_num;
    }
  },
  threading::accumulated_task_sizes([&](const IndexRange range) { return src_faces[range].size(); }));

  ensure_min_face_sizes({src_faces, src_corner_verts}, src_mesh.faces_num, new_faces_sizes, verts_to_dissolve_selection);

  const IndexMask verts_to_dissolve_mask = IndexMask::from_bools(boundary_verts_to_dissolve_mask, verts_to_dissolve_selection, memory);
  const IndexMask keeped_verts_mask = verts_to_dissolve_mask.complement(IndexMask(src_mesh.verts_num), memory);
  const IndexMask keeped_corners_mask = IndexMask::from_predicate(IndexMask(src_mesh.corners_num), GrainSize(4096), memory, [&](const int corner_i) {
  ...
  const OffsetIndices<int> dst_faces = offset_indices::accumulate_counts_to_offsets(new_faces_sizes);
  ...
  threading::parallel_invoke(src_mesh.edges_num + src_mesh.faces_num > 1000,
    [&](){
      unique_edges = dissolved_edges_for_verts(src_edges, vert_to_edge_map, verts_to_dissolve_mask, keeped_verts_mask, old_to_new_edges_map, new_edges);
    },
    [&]() {
      unique_faces = deduplicate_faces_for_verts({dst_faces, corner_verts}, old_to_new_faces_map, old_to_new_corners_map);
    });
  ...
  const OffsetIndices<int> unique_dst_faces = offset_indices::accumulate_counts_to_offsets(unique_face_sizes);

  Mesh *dst_mesh = BKE_mesh_new_nomain(keeped_verts_mask.size(), total_unique_edges, total_unique_faces, unique_dst_faces.total_size());
  BKE_mesh_copy_parameters_for_eval(dst_mesh, &src_mesh);

  const bke::AttributeAccessor src_attributes = src_mesh.attributes();
  bke::MutableAttributeAccessor dst_attributes = dst_mesh->attributes_for_write();

  bke::gather_attributes(src_attributes, bke::AttrDomain::Point, propagation_info, {}, keeped_verts_mask, dst_attributes);

  MutableSpan<int2> dst_edges = dst_mesh->edges_for_write();
  array_utils::copy(unique_edges.as_span().cast<int2>(), dst_edges);
  array_utils::gather(new_verts_remap.as_span(), dst_edges.as_span().cast<int>(), dst_edges.cast<int>());
  BLI_assert(!dst_edges.cast<int>().as_span().contains(-1));

  mix_attributes(src_attributes, bke::AttrDomain::Edge, propagation_info, {".edge_verts"}, old_to_new_edges_map, dst_attributes);

  array_utils::copy(unique_dst_faces.data(), dst_mesh->face_offsets_for_write());
  mix_attributes(src_attributes, bke::AttrDomain::Face, propagation_info, { /*"ID", "material_index"*/ }, old_to_new_faces_map, dst_attributes);
  // bke::gather_attributes(src_attributes, bke::AttrDomain::Face, propagation_info, {}, unique_faces.as_span(), dst_attributes);

  mix_attributes(src_attributes, bke::AttrDomain::Corner, propagation_info, {".corner_vert", ".corner_edge"}, old_to_new_corners_map, dst_attributes);
  ...
  threading::parallel_for(IndexRange(total_unique_faces), 2048, [&](const IndexRange range) {
    for (const int face_i : range) {
      const IndexRange face = unique_dst_faces[face_i];
      ...
      for (const int corner_i : face.index_range().drop_back(1)) {
        corner_edges[corner_i] = unique_edges.index_of(OrderedEdge(corner_verts[corner_i], corner_verts[corner_i + 1]));
      }
      corner_edges.last() = unique_edges.index_of(OrderedEdge(corner_verts.last(), corner_verts.first()));
    }
  },
  threading::accumulated_task_sizes([&](const IndexRange range) { return dst_faces[range].size(); }));
