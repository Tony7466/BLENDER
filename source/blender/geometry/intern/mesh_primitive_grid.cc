                          ...
                          const bke::AttributeIDRef &uv_map_id)
{
  ...
  threading::parallel_for_memory_bandwidth_bound(
      corner_verts.index_range(), 1024, [&](IndexRange range) {
                       ...
                       const bke::AttributeIDRef &uv_map_id)
{
...

  {
    ...
    threading::parallel_for_memory_bandwidth_bound(
        IndexRange(verts_x), 512, [&](IndexRange x_range) {
          for (const int x : x_range) {
            const int y_offset = x * verts_y;
            threading::parallel_for_memory_bandwidth_bound(
                IndexRange(verts_y), 512, [&](IndexRange y_range) {
  ...
  threading::parallel_for_memory_bandwidth_bound(
      IndexRange(verts_x), 512, [&](IndexRange x_range) {
        for (const int x : x_range) {
          ...
          threading::parallel_for_memory_bandwidth_bound(
              IndexRange(edges_y), 512, [&](IndexRange y_range) {
                for (const int y : y_range) {
  ...
  threading::parallel_for_memory_bandwidth_bound(
      IndexRange(verts_y), 512, [&](IndexRange y_range) {
        for (const int y : y_range) {
          ...
          threading::parallel_for_memory_bandwidth_bound(
              IndexRange(edges_x), 512, [&](IndexRange x_range) {
                for (const int x : x_range) {
