...
static void split_mask_for_ranges(const OffsetIndices<int> groups, const IndexMask mask, MutableSpan<IndexMask> masks)
{
...
{
  int offset_i = 0;
  mask.foreach_range([&](const IndexRange range) { r_offsets[offset_i] = range.size(); offset_i++; });
}
    ...
    const bke::AnonymousAttributePropagationInfo &propagation_info)
{
  ...
  const OffsetIndices<int> curves_subdivision_offset = offset_indices::accumulate_counts_to_offsets(curves_subdivision);
  ...
  bke::gather_attributes(src_attributes, bke::AttrDomain::Point, propagation_info, {}, points_mask, dst_attributes);
  bke::gather_attributes(src_attributes, bke::AttrDomain::Curve, propagation_info, {}, curve_indices.as_span(), dst_attributes);
