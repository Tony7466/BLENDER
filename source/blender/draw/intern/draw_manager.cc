...
void Manager::begin_sync()
{
  ...
  matrix_buf.current().optimize_size(resource_len_);
  bounds_buf.current().optimize_size(resource_len_);
  infos_buf.current().optimize_size(resource_len_);
  attributes_buf.optimize_size(attribute_len_);
