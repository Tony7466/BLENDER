...
static void statvis_calc_thickness(const MeshRenderData &mr, float *r_thickness)
{
  ...
  if (mr.extract_type == MR_EXTRACT_BMESH) {
    ...
    const Span<int> tri_faces = mr.tri_faces;
  ...
  if (mr.extract_type == MR_EXTRACT_BMESH) {
    ...
    if (overlap) {
      for (int i = 0; i < overlap_len; i++) {
        ...
        for (const IndexRange f_hit : {mr.faces[mr.tri_faces[overlap[i].indexA]],
                                       mr.faces[mr.tri_faces[overlap[i].indexB]]})
        {
