      ...
      switch (type) {
        ...
        case CD_PROP_FLOAT2: {
          if (layer != -1 && domain.has_value()) {
            drw_attributes_add_request(attributes, name, nullptr, type, layer, *domain);
          }
          break;
        }
        default:
    ...
    if (name) {
      ...
      if (drw_custom_data_match_attribute(cd_vdata, name, &layer_index, &type)) {
        drw_attributes_add_request(
            &attributes, name, nullptr, type, layer_index, bke::AttrDomain::Point);
      }
      else if (drw_custom_data_match_attribute(cd_ldata, name, &layer_index, &type)) {
        drw_attributes_add_request(
            &attributes, name, nullptr, type, layer_index, bke::AttrDomain::Corner);
      }
    }
  };
