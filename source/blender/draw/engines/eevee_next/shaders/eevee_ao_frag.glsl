...
vec3 view_position_derivative_from_depth(vec2 uvs, vec2 ofs, vec3 vP, float depth_center)
{
  ...
  vec4 H;
  H.x = textureLod(hiz_tx, uv1, 0.0).r;
  H.y = textureLod(hiz_tx, uv2, 0.0).r;
  H.z = textureLod(hiz_tx, uv3, 0.0).r;
  H.w = textureLod(hiz_tx, uv4, 0.0).r;
...
bool reconstruct_view_position_and_normal_from_depth(vec2 uvs, out vec3 vP, out vec3 vNg)
{
  ...
  float depth_center = textureLod(hiz_tx, uvs, 0.0).r;
...
void main()
{
  ...
  if (!reconstruct_view_position_and_normal_from_depth(uvs * hiz_buf.uv_scale, vP, vNg)) {
