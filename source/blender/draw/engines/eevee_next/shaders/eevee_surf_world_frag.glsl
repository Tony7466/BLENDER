...
void main()
{
  ...
  output_renderpass_color(g_buf.render_pass.environment_id, environment);

  vec4 clear_color = vec4(0.0, 0.0, 0.0, 1.0);
  output_renderpass_color(g_buf.render_pass.normal_id, clear_color);
  output_renderpass_color(g_buf.render_pass.diffuse_light_id, clear_color);
  output_renderpass_color(g_buf.render_pass.specular_light_id, clear_color);
  output_renderpass_color(g_buf.render_pass.diffuse_color_id, clear_color);
  output_renderpass_color(g_buf.render_pass.specular_color_id, clear_color);
  output_renderpass_color(g_buf.render_pass.emission_id, clear_color);
  output_renderpass_value(g_buf.render_pass.shadow_id, 1.0);
