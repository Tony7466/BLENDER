...
{
#if defined(MAT_RENDER_PASS_SUPPORT) && defined(GPU_FRAGMENT_SHADER)
  for (int i = 0; i < AOV_MAX && i < g_buf.render_pass.aovs.color_len; i++) {
    output_renderpass_color(g_buf.render_pass.color_len + i, vec4(0));
  }
  for (int i = 0; i < AOV_MAX && i < g_buf.render_pass.aovs.value_len; i++) {
    output_renderpass_value(g_buf.render_pass.value_len + i, 0.0);
  }
#endif
}
...
#if defined(MAT_RENDER_PASS_SUPPORT) && defined(GPU_FRAGMENT_SHADER)
  for (int i = 0; i < AOV_MAX && i < g_buf.render_pass.aovs.color_len; i++) {
    if (g_buf.render_pass.aovs.hash_color[i].x == hash) {
      imageStore(rp_color_img, ivec3(ivec2(gl_FragCoord.xy), g_buf.render_pass.color_len + i), color);
      return;
    }
  }
  for (int i = 0; i < AOV_MAX && i < g_buf.render_pass.aovs.value_len; i++) {
    if (g_buf.render_pass.aovs.hash_value[i].x == hash) {
      imageStore(rp_value_img, ivec3(ivec2(gl_FragCoord.xy), g_buf.render_pass.value_len + i), vec4(value));
      return;
    }
  }
#endif
...
vec3 coordinate_screen(vec3 P)
{
  ...
  if (false /* probe */) {
    ...
    window.xy = window.xy * g_buf.camera.uv_scale + g_buf.camera.uv_bias;
  }
  return window;
}
