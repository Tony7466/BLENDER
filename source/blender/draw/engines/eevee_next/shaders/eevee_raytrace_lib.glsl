...
void raytrace_screenspace_ray_finalize(inout ScreenSpaceRay ray)
{
  ...
  bool is_more_vertical = abs(ray.direction.x / rt_buf.pixel_size.x) <
                          abs(ray.direction.y / rt_buf.pixel_size.y);
  ray.direction /= (is_more_vertical) ? abs(ray.direction.y) : abs(ray.direction.x);
  ray.direction *= (is_more_vertical) ? rt_buf.pixel_size.y : rt_buf.pixel_size.x;
              ...
              out vec3 hit_position)
{
  ...
  for (float iter = 1.0; !hit && (time < ssray.max_time) && (iter < max_steps); iter++) {
    ...
    depth_sample = textureLod(hiz_tx, ss_p.xy * hiz_buf.uv_scale, floor(lod)).r;
