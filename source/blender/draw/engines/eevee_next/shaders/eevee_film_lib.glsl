...
float film_luma_weight(float luma)
{
  ...
  return 1.0 / (4.0 + luma * g_buf.film.exposure_scale);
}
...
#  ifdef SCALED_RENDERING
  texel_film /= g_buf.film.scaling_factor;
#  endif

  FilmSample film_sample = g_buf.film.samples[sample_n];
  film_sample.texel += texel_film + g_buf.film.render_offset;
  /* Use extend on borders. */
  film_sample.texel = clamp(film_sample.texel, ivec2(0, 0), g_buf.film.render_extent - 1);
...
#  ifdef SCALED_RENDERING
  ...
  vec2 offset = g_buf.film.subpixel_offset - vec2(texel_film % g_buf.film.scaling_factor);
  film_sample.weight = film_filter_weight(g_buf.film.filter_size, len_squared(offset));
#  endif
...
float film_weight_accumulation(ivec2 texel_film)
{
  ...
  float weight = 0.0;
  for (int i = 0; i < g_buf.film.samples_len; i++) {
    weight += film_sample_get(i, texel_film).weight;
  }
  return weight;
#endif
  return g_buf.film.samples_weight_total;
}
...
void film_sample_accum_mist(FilmSample samp, inout float accum)
{
  ...
  /* Remap to 0..1 range. */
  mist = saturate(mist * g_buf.film.mist_scale + g_buf.film.mist_bias);
  /* Falloff. */
  mist = pow(mist, g_buf.film.mist_exponent);
  accum += mist * samp.weight;
}

void film_sample_accum_combined(FilmSample samp, inout vec4 accum, inout float weight_accum)
{
  if (g_buf.film.combined_id == -1) {
    return;
  }
...
float film_distance_load(ivec2 texel)
{
  ...
  if (!g_buf.film.use_history || g_buf.film.use_reprojection) {
    return 1.0e16;
  }
...
vec2 film_pixel_history_motion_vector(ivec2 texel_sample)
{
  ...
  vector.xy *= vec2(g_buf.film.extent);
...
vec4 film_sample_catmull_rom(sampler2D color_tx, vec2 input_texel)
{
  ...
  vec2 uv_12 = (center_texel + weights[2] / weight_12) * g_buf.film.extent_inv;
  vec2 uv_0 = (center_texel - 1.0) * g_buf.film.extent_inv;
  vec2 uv_3 = (center_texel + 2.0) * g_buf.film.extent_inv;
                                ...
                                float luma_history)
{
  ...
  if (any(lessThan(texel, vec2(0))) || any(greaterThanEqual(texel, vec2(g_buf.film.extent)))) {
    blend = 1.0;
  }
  /* Discard history if invalid. */
  if (g_buf.film.use_history == false) {
    blend = 1.0;
  }
  return blend;
}
    ...
    FilmSample dst, ivec2 src_texel, vec4 color, float color_weight, inout vec4 display)
{
  ...
  if (g_buf.film.use_reprojection) {
...
void film_store_color(FilmSample dst, int pass_id, vec4 color, inout vec4 display)
{
  ...
  if (g_buf.film.display_id == pass_id) {
    display = color;
  }
...
void film_process_data(ivec2 texel_film, out vec4 out_color, out float out_depth)
{
  ...
  if (g_buf.film.combined_id != -1) {
    ...
    FilmSample src;
    for (int i = g_buf.film.samples_len - 1; i >= 0; i--) {
  ...
  if (g_buf.film.has_data) {
    ...
    if (g_buf.film.use_reprojection || film_sample.weight < film_distance) {
      vec4 normal = texelFetch(rp_color_tx, ivec3(film_sample.texel, g_buf.render_pass.normal_id), 0);
      float depth = texelFetch(depth_tx, film_sample.texel, 0).x;
      vec4 vector = velocity_resolve(vector_tx, film_sample.texel, depth);
      /* Transform to pixel space. */
      vector *= vec4(vec2(g_buf.film.render_extent), -vec2(g_buf.film.render_extent));

      film_store_depth(texel_film, depth, out_depth);
      film_store_data(texel_film, g_buf.film.normal_id, normal, out_color);
      film_store_data(texel_film, g_buf.film.vector_id, vector, out_color);
      film_store_distance(texel_film, film_sample.weight);
    }
    else {
      out_depth = imageLoad(depth_img, texel_film).r;
      if (g_buf.film.display_id != -1 && g_buf.film.display_id == g_buf.film.normal_id) {
        out_color = imageLoad(color_accum_img, ivec3(texel_film, g_buf.film.display_id));
      }
    }
  }

  if (g_buf.film.any_render_pass_1) {
    ...
    for (int i = 0; i < g_buf.film.samples_len; i++) {
      ...
      film_sample_accum(src,
                        g_buf.film.diffuse_light_id,
                        g_buf.render_pass.diffuse_light_id,
                        rp_color_tx,
                        diffuse_light_accum);
      film_sample_accum(src,
                        g_buf.film.specular_light_id,
                        g_buf.render_pass.specular_light_id,
                        rp_color_tx,
                        specular_light_accum);
      film_sample_accum(
          src, g_buf.film.volume_light_id, g_buf.render_pass.volume_light_id, rp_color_tx, volume_light_accum);
      film_sample_accum(
          src, g_buf.film.emission_id, g_buf.render_pass.emission_id, rp_color_tx, emission_accum);
    }
    film_store_color(dst, g_buf.film.diffuse_light_id, diffuse_light_accum, out_color);
    film_store_color(dst, g_buf.film.specular_light_id, specular_light_accum, out_color);
    film_store_color(dst, g_buf.film.volume_light_id, volume_light_accum, out_color);
    film_store_color(dst, g_buf.film.emission_id, emission_accum, out_color);
  }

  if (g_buf.film.any_render_pass_2) {
    ...
    for (int i = 0; i < g_buf.film.samples_len; i++) {
      FilmSample src = film_sample_get(i, texel_film);
      film_sample_accum(src,
                        g_buf.film.diffuse_color_id,
                        g_buf.render_pass.diffuse_color_id,
                        rp_color_tx,
                        diffuse_color_accum);
      film_sample_accum(src,
                        g_buf.film.specular_color_id,
                        g_buf.render_pass.specular_color_id,
                        rp_color_tx,
                        specular_color_accum);
      film_sample_accum(
          src, g_buf.film.environment_id, g_buf.render_pass.environment_id, rp_color_tx, environment_accum);
      film_sample_accum(src, g_buf.film.shadow_id, g_buf.render_pass.shadow_id, rp_value_tx, shadow_accum);
      film_sample_accum(
          src, g_buf.film.ambient_occlusion_id, g_buf.render_pass.ambient_occlusion_id, rp_value_tx, ao_accum);
      film_sample_accum_mist(src, mist_accum);
    }
    film_store_color(dst, g_buf.film.diffuse_color_id, diffuse_color_accum, out_color);
    film_store_color(dst, g_buf.film.specular_color_id, specular_color_accum, out_color);
    film_store_color(dst, g_buf.film.environment_id, environment_accum, out_color);
    film_store_color(dst, g_buf.film.shadow_id, vec4(vec3(shadow_accum), 1.0), out_color);
    film_store_color(dst, g_buf.film.ambient_occlusion_id, vec4(vec3(ao_accum), 1.0), out_color);
    film_store_value(dst, g_buf.film.mist_id, mist_accum, out_color);
  }

  for (int aov = 0; aov < g_buf.film.aov_color_len; aov++) {
    vec4 aov_accum = vec4(0.0);

    for (int i = 0; i < g_buf.film.samples_len; i++) {
      FilmSample src = film_sample_get(i, texel_film);
      film_sample_accum(src, 0, g_buf.render_pass.color_len + aov, rp_color_tx, aov_accum);
    }
    film_store_color(dst, g_buf.film.aov_color_id + aov, aov_accum, out_color);
  }

  for (int aov = 0; aov < g_buf.film.aov_value_len; aov++) {
    float aov_accum = 0.0;

    for (int i = 0; i < g_buf.film.samples_len; i++) {
      FilmSample src = film_sample_get(i, texel_film);
      film_sample_accum(src, 0, g_buf.render_pass.value_len + aov, rp_value_tx, aov_accum);
    }
    film_store_value(dst, g_buf.film.aov_value_id + aov, aov_accum, out_color);
  }

  if (g_buf.film.cryptomatte_samples_len != 0) {
    ...
    film_cryptomatte_layer_accum_and_store(
        dst, texel_film, g_buf.film.cryptomatte_object_id, 0, out_color);
    film_cryptomatte_layer_accum_and_store(
        dst, texel_film, g_buf.film.cryptomatte_asset_id, 1, out_color);
    film_cryptomatte_layer_accum_and_store(
        dst, texel_film, g_buf.film.cryptomatte_material_id, 2, out_color);
  }
}
