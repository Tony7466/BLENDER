...
void main(void)
{
  ...
  for (int i = 0; i < g_buf.subsurface.sample_len; i++) {
    vec2 sample_uv = center_uv + sample_space * g_buf.subsurface.samples[i].xy;
    float pdf_inv = g_buf.subsurface.samples[i].z;

    float sample_depth = textureLod(hiz_tx, sample_uv * g_buf.hiz.uv_scale, 0.0).r;
  ...
  if (g_buf.render_pass.diffuse_light_id >= 0) {
    imageStore(rp_color_img, ivec3(texel, g_buf.render_pass.diffuse_light_id), vec4(accum, 1.0));
  }
