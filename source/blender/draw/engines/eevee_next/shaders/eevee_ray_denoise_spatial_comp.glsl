...
void main()
{
  ...
  ivec2 texel = (texel_fullres) / g_buf.raytrace.resolution_scale;

  if (g_buf.raytrace.skip_denoise) {
  ...
  vec2 uv = (vec2(texel_fullres) + 0.5) * g_buf.raytrace.full_resolution_inv;
  ...
  if (g_buf.raytrace.resolution_scale > 1) {
  ...
  for (uint i = 0u; i < sample_count; i++) {
    ...
    ivec2 sample_tile = ivec2(sample_texel * g_buf.raytrace.resolution_scale) / int(tile_size);
