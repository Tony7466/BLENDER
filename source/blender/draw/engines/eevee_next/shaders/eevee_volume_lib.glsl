...
{
  float near = g_buf.volumes.depth_near;
  float far = g_buf.volumes.depth_far;
  float distribution = g_buf.volumes.depth_distribution;
...
vec3 volume_to_ndc(vec3 coord)
{
  ...
  coord.xy /= g_buf.volumes.coord_scale;
  return coord;
}
...
vec3 ndc_to_volume(vec3 coord)
{
  ...
  float far = g_buf.volumes.depth_far;
  float distribution = g_buf.volumes.depth_distribution;
  vec2 coord_scale = g_buf.volumes.coord_scale;
  /* Implemented in eevee_shader_shared.cc */
...
vec3 volume_light(LightData ld, vec3 L, float l_dist)
{
  ...
  if (ld.type != LIGHT_SUN) {
    ...
    float light_clamp = g_buf.volumes.light_clamp;
    if (light_clamp != 0.0) {
...
vec3 volume_shadow(LightData ld, vec3 ray_wpos, vec3 L, float l_dist, sampler3D extinction_tx)
{
  ...
  if (g_buf.volumes.shadow_steps == 0) {
    return vec3(1.0);
  }
...
#  if 0
  ...
  if (g_buf.volumes.use_soft_shadows && ld.shadowid >= 0.0) {
...
#  endif
  ...
  float dd = l_vector.w / g_buf.volumes.shadow_steps;
  L = l_vector.xyz / g_buf.volumes.shadow_steps;
  ...
  if (is_sun_light(ld.type)) {
    ...
    L /= g_buf.volumes.shadow_steps;
    dd = length(L);
  }

  /* TODO use shadow maps instead. */
  vec3 shadow = vec3(1.0);
  for (float s = 1.0; s < VOLUMETRIC_SHADOW_MAX_STEP && s <= g_buf.volumes.shadow_steps;
       s += 1.0) {
