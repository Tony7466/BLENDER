                                               ...
                                               const float sample_count)
{
  ...
  for (float iter = 0.0; time < ssray.max_time && iter < sample_count; iter++) {
    ...
    float lod = (log2(stride) - noise) / (1.0 + g_buf.ao.quality);

    vec2 uv = ssray.origin.xy + ssray.direction.xy * time;
    float depth = textureLod(depth_tx, uv * g_buf.hiz.uv_scale, floor(lod)).r;
                                       ...
                                       const float dir_sample_count)
{
  ...
  for (int i = 0; i < 2; i++) {
    ...
    ssray = raytrace_screenspace_ray_create(ray, g_buf.ao.pixel_size);
