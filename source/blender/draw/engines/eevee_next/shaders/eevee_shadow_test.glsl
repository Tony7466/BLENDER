...
void main()
{
  ...
  TEST(eevee_shadow, DirectionalClipmapCoordinates)
  {
    ...
    EXPECT_EQ(coords.tilemap_tile, ivec2(SHADOW_TILEMAP_RES / 2));
    // EXPECT_NEAR(coords.uv, vec2(SHADOW_TILEMAP_RES / 2), 1e-3);

    lP = vec3(-1e-5, -1e-5, 0.0);
    coords = shadow_directional_coordinates(light, lP);
    EXPECT_EQ(coords.tilemap_index, 0);
    EXPECT_EQ(coords.tilemap_tile, ivec2((SHADOW_TILEMAP_RES / 2) - 1));
    // EXPECT_NEAR(coords.uv, vec2(SHADOW_TILEMAP_RES / 2), 1e-3);

    lP = vec3(-0.5, -0.5, 0.0); /* Min of first LOD. */
    coords = shadow_directional_coordinates(light, lP);
    EXPECT_EQ(coords.tilemap_index, 0);
    EXPECT_EQ(coords.tilemap_tile, ivec2(0));
    // EXPECT_NEAR(coords.uv, vec2(0), 1e-3);

    lP = vec3(0.5, 0.5, 0.0); /* Max of first LOD. */
    coords = shadow_directional_coordinates(light, lP);
    EXPECT_EQ(coords.tilemap_index, 0);
    EXPECT_EQ(coords.tilemap_tile, ivec2(SHADOW_TILEMAP_RES - 1));
    ...
    EXPECT_EQ(coords.tilemap_tile, ivec2(SHADOW_TILEMAP_RES / 4));
    ...
    coords = shadow_directional_coordinates(light, lP);
    EXPECT_EQ(coords.tilemap_tile, ivec2(SHADOW_TILEMAP_RES / 2) + ivec2(1, -1));

    coords = shadow_directional_coordinates(light, lP);
    EXPECT_EQ(coords.tilemap_tile, ivec2(SHADOW_TILEMAP_RES / 2) + ivec2(1, 0));
    ...
    coords = shadow_directional_coordinates(light, lP);
    EXPECT_EQ(coords.tilemap_tile, ivec2(SHADOW_TILEMAP_RES / 2 - 1) + ivec2(-1, 1));

    coords = shadow_directional_coordinates(light, lP);
    EXPECT_EQ(coords.tilemap_tile, ivec2(SHADOW_TILEMAP_RES / 2 - 1) + ivec2(-1, 0));
  ...
  TEST(eevee_shadow, DirectionalCascadeCoordinates)
  {
    ...
    coords = shadow_directional_coordinates(light, lP);
    EXPECT_EQ(coords.tilemap_index, 1);
    EXPECT_EQ(coords.tilemap_tile, ivec2(SHADOW_TILEMAP_RES - 1, SHADOW_TILEMAP_RES / 2));
