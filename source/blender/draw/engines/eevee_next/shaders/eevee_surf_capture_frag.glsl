  ...
  if (capture_info_buf.do_surfel_count) {
    ...
    if (is_surface_view_aligned) {
      ...
      if (capture_info_buf.do_surfel_output) {
        ...
        surfel_buf[surfel_id].albedo_back = double_sided_for_gi ? albedo : vec3(0);
        surfel_buf[surfel_id].radiance_direct.back.rgb = double_sided_for_gi ? g_emission :
                                                                               vec3(0);
        surfel_buf[surfel_id].radiance_direct.back.a = 0.0;
        surfel_buf[surfel_id].double_sided = double_sided_for_gi;
