...
void main()
{
  ...
  output_renderpass_color(g_buf.render_pass.normal_id, vec4(out_normal, 1.0));
  output_renderpass_color(g_buf.render_pass.diffuse_color_id, vec4(g_diffuse_data.color, 1.0));
  output_renderpass_color(g_buf.render_pass.diffuse_light_id, vec4(diffuse_light, 1.0));
  output_renderpass_color(g_buf.render_pass.specular_color_id, vec4(specular_color, 1.0));
  output_renderpass_color(g_buf.render_pass.specular_light_id, vec4(specular_light, 1.0));
  output_renderpass_color(g_buf.render_pass.emission_id, vec4(g_emission, 1.0));
  output_renderpass_value(g_buf.render_pass.shadow_id, shadow);
  /** NOTE: AO is done on its own pass. */
#endif

#ifdef MAT_TRANSPARENT
  /* Volumetric resolve and compositing. */
  vec2 uvs = gl_FragCoord.xy * g_buf.volumes.viewport_size_inv;
