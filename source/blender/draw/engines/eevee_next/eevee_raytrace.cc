                                      ...
                                      View &render_view)
{
  ...
  auto bias_get = [&](int scale_factor) -> int2 {
    if (scale_factor == 1) {
      return int2(0);
    }
    int pixel_count = square_i(scale_factor);
    int prime = (pixel_count / 2) - 1;
    int index_random = int((inst_.sampling.sample_index() * prime) % pixel_count);
    return int2(index_random % scale_factor, index_random / scale_factor);
  };

  /* Data for the radiance setup. */
  data_.resolution_scale = resolution_scale;
  data_.resolution_bias = bias_get(resolution_scale);
  ...
  data_.horizon_resolution_bias = bias_get(horizon_resolution_scale);
