                             ...
                             ExtraInstanceData data)
{
  ...
  switch (pd->forcefield) {
    case PFIELD_FORCE:
      pass.field_force.append(data, select_id);
      break;
    case PFIELD_WIND:
      size_z = pd->f_strength;
      pass.field_wind.append(data, select_id);
      break;
    case PFIELD_VORTEX:
      size_y = (pd->f_strength < 0.0f) ? -size_y : size_y;
      pass.field_vortex.append(data, select_id);
      break;
    case PFIELD_GUIDE:
      if (cu && (cu->flag & CU_PATH) && ob->runtime.curve_cache->anim_path_accum_length) {
        ...
        pass.field_curve.append(data.with_matrix(matrix), select_id);

        BKE_where_on_path(ob, 1.0f, position, nullptr, nullptr, nullptr, nullptr);
        matrix.location() = data.matrix.location() + position.xyz();
        pass.field_sphere_limit.append(data.with_matrix(matrix), select_id);
      }
      break;
  }
  ...
  if (pd->falloff == PFIELD_FALL_TUBE) {
    if (pd->flag & (PFIELD_USEMAX | PFIELD_USEMAXR)) {
      ...
      size_y = size_x;
      pass.field_tube_limit.append(data, select_id);
    }
    ...
    if (pd->flag & (PFIELD_USEMAX | PFIELD_USEMAXR)) {
      ...
      size_y = size_x;
      pass.field_cone_limit.append(data, select_id);
    }
    if (pd->flag & (PFIELD_USEMIN | PFIELD_USEMINR)) {
    ...
    if (pd->flag & PFIELD_USEMAX) {
      ...
      pass.field_sphere_limit.append(data, select_id);
    }
