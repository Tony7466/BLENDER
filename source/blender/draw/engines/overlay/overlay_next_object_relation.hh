...
static void object_relation_sync(const ObjectRef &ob_ref,
                                 ...
                                 ExtraInstancePass &pass)
{
  ...
  if (ob->parent && (DRW_object_visibility_in_active_context(ob->parent) & OB_VISIBLE_SELF)) {
    ...
    pass.relation_line.append({parent_pos, ob_pos}, select_id);
  }
  ...
  for (ModifierData *md : ListBaseWrapper<ModifierData>(&ob->modifiers)) {
    if (md->type == eModifierType_Hook) {
      ...
      if (hmd->object) {
        pass.relation_line.append({hmd->object->object_to_world[3], center}, select_id);
      }
      pass.relation_point.append(float4(center), select_id);
    }
  }
  for (GpencilModifierData *md : ListBaseWrapper<GpencilModifierData>(ob->greasepencil_modifiers))
  {
  ...
  if (ob->rigidbody_constraint) {
    ...
    if (rbc_ob1 && (DRW_object_visibility_in_active_context(rbc_ob1) & OB_VISIBLE_SELF)) {
      pass.relation_line.append({rbc_ob1->object_to_world[3], ob_pos}, select_id);
    }
    if (rbc_ob2 && (DRW_object_visibility_in_active_context(rbc_ob2) & OB_VISIBLE_SELF)) {
      pass.relation_line.append({rbc_ob2->object_to_world[3], ob_pos}, select_id);
    }
  }
  ...
  if (!BLI_listbase_is_empty(&ob->constraints)) {
    ...
    for (bConstraint *con : ListBaseWrapper<bConstraint>(&ob->constraints)) {
      if (ELEM(con->type, CONSTRAINT_TYPE_FOLLOWTRACK, CONSTRAINT_TYPE_OBJECTSOLVER)) {
        ...
        if (cam_ob) {
          pass.constraint_line.append({cam_ob->object_to_world[3], ob_pos}, select_id);
        }
      }
      else {
        ...
        if ((con->ui_expand_flag & (1 << 0)) && BKE_constraint_targets_get(con, &targets)) {
          ...
          for (bConstraintTarget *target : ListBaseWrapper<bConstraintTarget>(&targets)) {
            ...
            pass.constraint_line.append({target_pos, ob_pos}, select_id);
          }
