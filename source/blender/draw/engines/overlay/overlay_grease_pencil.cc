...
void OVERLAY_edit_grease_pencil_cache_populate(OVERLAY_Data *vedata, Object *ob)
{
  ...
  if (lines_grp) {
    GPUBatch *geom_lines = DRW_cache_grease_pencil_edit_lines_get(ob, draw_ctx->scene);
  ...
  if (points_grp) {
    GPUBatch *geom_points = DRW_cache_grease_pencil_edit_points_get(ob, draw_ctx->scene);
