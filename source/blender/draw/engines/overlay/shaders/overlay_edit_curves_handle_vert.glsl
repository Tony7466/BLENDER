...
void main()
{
  ...
  if ((data & EDIT_CURVES_BEZIER_HANDLE) != 0u) {
    leftColor = get_bezier_handle_color((data >> (EDIT_CURVES_HANDLE_TYPES_SHIFT + 2)) & 3, selection);
    finalColor = get_bezier_handle_color((data >> EDIT_CURVES_HANDLE_TYPES_SHIFT) & 3, selection);
