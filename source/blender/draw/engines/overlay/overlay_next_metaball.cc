...
void Metaballs::begin_sync()
{
  circle_buf_.clear();
  circle_in_front_buf_.clear();
}

void Metaballs::edit_object_sync(const ObjectRef &ob_ref, Resources &res)
{
...
void Metaballs::object_sync(const ObjectRef &ob_ref, Resources &res, const State &state)
{
  SphereOutlineInstanceBuf &circle_buf = (ob_ref.object->dtx & OB_DRAW_IN_FRONT) != 0 ?
                                             circle_in_front_buf_ :
                                             circle_buf_;
...
void Metaballs::end_sync(Resources &res, ShapeCache &shapes, const State &state)
{
...
void Metaballs::draw(Resources &res, Manager &manager, View &view)
{
  GPU_framebuffer_bind(res.overlay_line_fb);
  manager.submit(metaball_ps_, view);
}

void Metaballs::draw_in_front(Resources &res, Manager &manager, View &view)
{
