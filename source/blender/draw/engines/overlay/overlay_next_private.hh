...
template<typename PassHandler, typename Pass = PassSimple> class OverlayType {
 private:
  const SelectionType selection_type_;

  PassHandler ps_handler;
  typename PassHandler::CallBuffers call_buffers_[2] = {{selection_type_}, {selection_type_}};

  Pass ps_ = {ps_handler.name};
  std::string in_front_name_ = std::string(ps_handler.name) + "_in_front";
  Pass ps_in_front_ = {in_front_name_.c_str()};

 public:
  OverlayType(SelectionType selection_type_) : selection_type_(selection_type_){};

  void begin_sync()
  {
    ps_handler.begin_sync(call_buffers_[0], ps_, false);
    ps_handler.begin_sync(call_buffers_[1], ps_in_front_, true);
  }

  void object_sync(const ObjectRef &ob_ref, Resources &res, const State &state)
  {
    const bool in_front = (ob_ref.object->dtx & OB_DRAW_IN_FRONT) != 0;
    typename PassHandler::CallBuffers &call_buffers = in_front ? call_buffers_[1] :
                                                                 call_buffers_[0];
    Pass &pass = in_front ? ps_in_front_ : ps_;
    ps_handler.object_sync(ob_ref, res, state, call_buffers, pass, in_front);
  }

  void end_sync(Resources &res, ShapeCache &shapes, const State &state)
  {
    ps_handler.end_sync(res, shapes, state, call_buffers_[0], ps_, false);
    ps_handler.end_sync(res, shapes, state, call_buffers_[1], ps_in_front_, true);
  }
  ...
  void draw_in_front(Resources &res, Manager &manager, View &view)
  {
    ...
    manager.submit(ps_in_front_, view);
  }
};
