...
{
  GPUShader *shader = context.get_shader("compositor_jump_flooding", DataPrecision::Half);
...
{
  BLI_assert(input.type() == DataType::Int2);
  BLI_assert(output.type() == DataType::Int2);
  ...
  Result initial_flooded_result = context.create_result(DataType::Int2, DataPrecision::Half);
  ...
  Result intermediate_result = context.create_result(DataType::Int2, DataPrecision::Half);
