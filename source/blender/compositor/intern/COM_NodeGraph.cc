...
{
  add_bNodeTree(context, 0, tree, blender::bke::NODE_INSTANCE_KEY_BASE);
}
                              ...
                              bNodeInstanceKey parent_key)
{
  ...
  for (bNode *node = (bNode *)tree->nodes.first; node; node = node->next) {
    bNodeInstanceKey key = blender::bke::BKE_node_instance_key(parent_key, tree, node);
...
void NodeGraph::add_proxies_group_inputs(bNode *b_node, bNode *b_node_io)
{
  ...
  bNodeInstanceKey key = blender::bke::NODE_INSTANCE_KEY_BASE;
