...
/* -------------------------------------------------------------------- */
/** \name Scale to Average Operator
 * \{ */

static void scale_to_average_graph_keys(bAnimContext *ac, const float factor)
{
  ListBase anim_data = {NULL, NULL};
  ...
  LISTBASE_FOREACH (bAnimListElem *, ale, &anim_data) {
    ...
    LISTBASE_FOREACH (FCurveSegment *, segment, &segments) {
      scale_to_average_fcurve_segment(fcu, segment, factor);
    }
...
static void scale_to_average_draw_status_header(bContext *C, tGraphSliderOp *gso)
{
  char status_str[UI_MAX_DRAW_STR];
  char mode_str[32];
  char slider_string[UI_MAX_DRAW_STR];
  ...
  strcpy(mode_str, TIP_("Scale to Average Keys"));
...
static void scale_to_average_modal_update(bContext *C, wmOperator *op)
{
  tGraphSliderOp *gso = op->customdata;

  scale_to_average_draw_status_header(C, gso);

  /* Reset keyframes to the state at invoke. */
  reset_bezts(gso);
  const float factor = slider_factor_get_and_remember(op);
  scale_to_average_graph_keys(&gso->ac, factor);
  WM_event_add_notifier(C, NC_ANIMATION | ND_KEYFRAME | NA_EDITED, NULL);
}

static int scale_to_average_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  const int invoke_result = graph_slider_invoke(C, op, event);
  ...
  tGraphSliderOp *gso = op->customdata;
  gso->modal_update = scale_to_average_modal_update;
  gso->factor_prop = RNA_struct_find_property(op->ptr, "factor");
  scale_to_average_draw_status_header(C, gso);
...
static int scale_to_average_exec(bContext *C, wmOperator *op)
{
  bAnimContext ac;
  ...
  scale_to_average_graph_keys(&ac, factor);
...
void GRAPH_OT_scale_to_average(wmOperatorType *ot)
{
  /* Identifiers. */
  ot->name = "Scale to Average Keyframes";
  ot->idname = "GRAPH_OT_scale_to_average";
  ot->description = "Increase or decrease the value of selected keys \n\
  in relationship to their average";

  /* API callbacks. */
  ot->invoke = scale_to_average_invoke;
  ot->modal = graph_slider_modal;
  ot->exec = scale_to_average_exec;
  ot->poll = graphop_editable_keyframes_poll;
