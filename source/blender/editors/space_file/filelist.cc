...
int ED_path_extension_type(const char *path)
{
  ...
  /* NOTE: While `.ttc` files can be loaded, only a single "face" is supported,
   * users will have to extract bold/italic ... etc manually for Blender to use them,
   * see #44254. */
  if (BLI_path_extension_check_n(path, ".ttf", ".pfb", ".otf", ".otc", ".woff", ".woff2", nullptr))
  {
