...
static void area_actionzone_get_rect(AZone *az, rcti *rect)
{
  if (az->type == AZONE_REGION_SCROLL) {
    ...
    const bool is_right = bool(az->region->v2d.scroll & V2D_SCROLL_RIGHT);
    const bool is_left = bool(az->region->v2d.scroll & V2D_SCROLL_LEFT);
    const bool is_top = bool(az->region->v2d.scroll & V2D_SCROLL_TOP);
    const bool is_botton = bool(az->region->v2d.scroll & V2D_SCROLL_BOTTOM);
    ...
    rect->xmin = scroller_vert.xmin - (is_vertical && is_right ? V2D_SCROLL_HIDE_HEIGHT : 0);
    rect->ymin = scroller_vert.ymin - (is_horizontal && is_top ? V2D_SCROLL_HIDE_WIDTH : 0);
    rect->xmax = scroller_vert.xmax + (is_vertical && is_left ? V2D_SCROLL_HIDE_HEIGHT : 0);
    rect->ymax = scroller_vert.ymax + (is_horizontal && is_botton ? V2D_SCROLL_HIDE_WIDTH : 0);
  }
  else {
