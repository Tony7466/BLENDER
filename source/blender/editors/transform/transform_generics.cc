...
void initTransInfo(bContext *C, TransInfo *t, wmOperator *op, const wmEvent *event)
{
...

  {
     ...
     * (#OperatorFlag::Repeat), reusing the orientation prevents unpredictable changes that can
     * occur when using #V3D_ORIENT_VIEW. However, when activated by #SCREEN_OT_repeat_last
     * (#OperatorFlag::RepeatLast), it's best to avoid reusing the orientation to prevent
     * unintended changes. */
    if (op && !bool(op->flag & OperatorFlag::RepeatLast) &&
