                                   ...
                                   float out[3])
{
  ...
  if (!td && t->con.mode & CON_APPLY) {
    ...
    if (transform_snap_is_active(t)) {
      if (validSnap(t)) {
        is_snap_to_edge = (t->tsnap.target_type & SCE_SNAP_MODE_EDGE) != 0;
        is_snap_to_face = (t->tsnap.target_type & SCE_SNAP_MODE_FACE) != 0;
        is_snap_to_point = !is_snap_to_edge && !is_snap_to_face;
      }
      else if (t->tsnap.target_type & SCE_SNAP_MODE_GRID) {
