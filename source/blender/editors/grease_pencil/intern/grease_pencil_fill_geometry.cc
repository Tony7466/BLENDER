...
/**
 * Explanation of the used algorithm for geometry-based fill:
 *
 *
 *     |                         (C)      segment 1       \ (D)
 *    -|-->>----------------------x--->>------------->>----\---
 ...
 public:
                                                            ...
                                                            const int segment_direction)
  {
    ...
    BLI_rctf_do_minmax_v(&bbox_segment,
                         this->curves_2d.points_2d[segment_point_offset + segment.point_range[0]]);
    BLI_rctf_do_minmax_v(&bbox_segment,
    ...
    threading::parallel_for(
        this->curves_2d.point_offset.index_range(), 256, [&](const IndexRange range) {
          for (const int curve_i : range) {
            ...
            for (const SegmentEnd &segment_end : segment.segment_ends) {
            ...
            if (has_previous_segment) {
              for (const SegmentEnd &segment_end : previous_segment.segment_ends) {
                if ((segment_end.flag & gfFlag::WithEndExtension) == false &&
                    segment_end.curve_index_2d == curve_i)
                {
                  skip_curve = true;
            ...
            for (const OverlappingSegment &overlap : this->overlapping_segments) {
  ...
  Vector<float3> get_closed_fill_edge_as_3d_points()
  {
    ...
    for (EdgeSegment &head : this->segments) {
    ...
    for (EdgeSegment &segment : this->segments) {
   ...
   */
  bool mouse_pos_is_inside_polygon(float2 &mouse_pos, Span<float2> polygon)
  {
  ...
  Vector<float2> get_closed_fill_edge_as_2d_polygon()
  {
    ...
    for (const EdgeSegment &segment : this->segments) {
      if ((segment.flag & gfFlag::IsUnused) == true) {
        continue;
      }
  ...
  void add_curves_in_proximity_of_segment(EdgeSegment &segment)
  {
    ...
    for (const ProximityCurve &proxim : proximities) {
      /* Copy proximity data. */
  ...
  void add_segment_ends_intersection(const Curves2DSpace &curves_2d,
                                     ...
                                     const Span<IntersectingCurve> intersections,
                                     const bool is_end_extension)
  {
    const float2 segment_vec = segment_point_b - segment_point_a;

    for (const IntersectingCurve &intersection : intersections) {
  ...
  bool walk_along_curve(EdgeSegment &segment, const int started_at = -1)
  {
    ...
    do {
      ...
      if (this->use_gap_close_extend) {
         ...
         */
        for (IntersectingCurve intersection : this->extension_intersections) {
    ...
    if (this->use_gap_close_extend) {
      ...
      for (const IntersectingCurve &intersection : this->extension_intersections) {
  ...
  void debug_print_segment_data(EdgeSegment &segment, const bool proximity_only)
  {
    ...
    for (const SegmentEnd &seg_end : segment.segment_ends) {
  ...
  bool execute_fill() override
  {
    ...
    for (gfRayDirection ray_dir : ray_directions) {
       ...
       * the curve for that. */
      const int point_offset = this->curves_2d.point_offset[segment.curve_index_2d];
      const float2 segment_a = this->curves_2d.points_2d[point_offset + start_segment.point_start];
