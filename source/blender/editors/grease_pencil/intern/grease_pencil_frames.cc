                            ...
                            bool &r_inserted_keyframe)
{
  ...
  if (blender::animrig::is_autokey_on(&scene) && needs_new_drawing) {
        ...
        (brush->gpencil_tool == GPAINT_TOOL_ERASE))
    {
      ...
      grease_pencil.insert_frame(active_layer, current_frame, 0, BEZT_KEYTYPE_KEYFRAME);
    }
  ...
  if (all_layers) {
    for (Layer *layer : grease_pencil.layers_for_write()) {
      ...
      changed |= grease_pencil.insert_frame(
                     *layer, current_frame, duration, BEZT_KEYTYPE_KEYFRAME) != nullptr;
    }
  }
  else {
    ...
    changed |= grease_pencil.insert_frame(*grease_pencil.get_active_layer(),
                                          current_frame,
                                          duration,
                                          BEZT_KEYTYPE_KEYFRAME) != nullptr;
  }
