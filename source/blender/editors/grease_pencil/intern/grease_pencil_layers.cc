...
static int grease_pencil_layer_add_exec(bContext *C, wmOperator *op)
{
  ...
  if (grease_pencil.has_active_layer()) {
    LayerGroup &active_group = grease_pencil.active_layer->base.parent->wrap();
    Layer &new_layer = grease_pencil.add_layer_after(
        active_group, &grease_pencil.active_layer->wrap(), new_layer_name);
    grease_pencil.active_layer = reinterpret_cast<GreasePencilLayer *>(&new_layer);
  }
  else {
