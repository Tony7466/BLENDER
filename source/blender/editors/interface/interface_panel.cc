...
static float layout_panel_y_val_in_scaled_block(const uiBlock *block, float val)
{
  const float aspect = ui_block_is_popup_any(block) ? block->aspect : 1.0f;
  return block->rect.ymax + (val - float(layout_panel_y_offset())) / aspect;
}
                                    ...
                                    float subpanel_backcolor[4])
{
  ...
  for (const LayoutPanelBody &body : panel->runtime->layout_panels.bodies) {
    ...
    panel_blockspace.ymin = layout_panel_y_val_in_scaled_block(block, body.start_y);
    panel_blockspace.ymax = layout_panel_y_val_in_scaled_block(block, body.end_y);
...
LayoutPanelHeader *ui_layout_panel_header_under_mouse(const Panel &panel, const int my)
{
  ...
  for (LayoutPanelHeader &header : panel.runtime->layout_panels.headers) {
    const float ymin = layout_panel_y_val_in_scaled_block(block, header.start_y);
    const float ymax = layout_panel_y_val_in_scaled_block(block, header.end_y);
    if (IN_RANGE(float(my), ymin, ymax)) {
      return &header;
    }
  }
  return nullptr;
                                   ...
                                   const int xy_dst[2])
{
  ...
  LISTBASE_FOREACH (uiBlock *, block, &region->uiblocks) {
    ...
    for (LayoutPanelHeader &header : panel->runtime->layout_panels.headers) {
      ...
      rect.ymin = layout_panel_y_val_in_scaled_block(block, header.start_y);
      rect.ymax = layout_panel_y_val_in_scaled_block(block, header.end_y);
