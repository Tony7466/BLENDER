    ...
    const uiBut *but, BIFIconID icon, float alpha, const rcti *rect, const uchar mono_color[4])
{
  ...
  if (ELEM(but->type, UI_BTYPE_TOGGLE, UI_BTYPE_ROW, UI_BTYPE_TOGGLE_N, UI_BTYPE_LISTROW)) {
    ...
    else if (but->flag & UI_HOVER_FOCUS) {
      /* pass */
    }
    else {
      alpha = 0.75f;
  ...
  if (icon && icon != ICON_BLANK1) {
    ...
    if (ui_but_drag_is_draggable(but) && (but->flag & UI_HOVER_FOCUS)) {
      UI_icon_draw_ex(
          xs, ys, icon, aspect, 1.25f, 0.0f, color, has_theme, &but->icon_overlay_text);
    }
    else if (but->flag & (UI_HOVER_FOCUS | UI_SELECT | UI_SELECT_DRAW)) {
      UI_icon_draw_ex(
...
static void widget_state(uiWidgetType *wt, const uiWidgetStateInfo *state, eUIEmbossType emboss)
{
  ...
  if (state->but_flag & UI_SELECT) {
     ...
     * position and so can draw without UI_HOVER_FOCUS set.  See D6503. */
    if (state->but_flag & UI_HOVER_FOCUS) {
                                       ...
                                       eUIEmbossType /*emboss*/)
{
  ...
  if ((state->but_flag & UI_BUT_DISABLED) && (state->but_flag & UI_HOVER_FOCUS)) {
    ...
    /* regular active */
    if (state->but_flag & (UI_SELECT | UI_HOVER_FOCUS)) {
      copy_v3_v3_uchar(wt->wcol.text, wt->wcol.text_sel);
    }
    ...
    else if (state->but_flag & UI_HOVER_FOCUS) {
      copy_v4_v4_uchar(wt->wcol.inner, wt->wcol.item);
    }
  }
}
                               ...
                               bool emboss)
{
  ...
  /* decoration */
  if ((state->but_flag & UI_HOVER_FOCUS) && !state->is_text_input) {
                               ...
                               const float zoom)
{
  ...
  if ((state->but_flag & UI_HOVER_FOCUS) || (back[3] < 1.0f)) {
    uiWidgetBase wtb;
    const float rad = widget_radius_from_zoom(zoom, wcol);
                                ...
                                const float zoom)
{
  ...
  if (state->but_flag & UI_HOVER_FOCUS && !(state->but_flag & UI_SELECT)) {
...
void ui_draw_but(const bContext *C, ARegion *region, uiStyle *style, uiBut *but, rcti *rect)
{
  ...
  if (but->block->flag & UI_BLOCK_POPOVER_ONCE) {
    if ((but->flag & UI_HOVER_FOCUS) && ui_but_is_popover_once_compat(but)) {
