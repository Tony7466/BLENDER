    ...
    case Type::Position: {
      ...
      if (use_multires_undo(step_data, ss)) {
        ...
        for (bke::pbvh::Node *node : nodes) {
          const Span<int> grids = bke::pbvh::node_grid_indices(
              static_cast<const bke::pbvh::GridsNode &>(*node));
        ...
        for (bke::pbvh::Node *node : nodes) {
          if (indices_contain_true(
                  modified_verts,
                  bke::pbvh::node_verts(static_cast<const bke::pbvh::MeshNode &>(*node))))
          {
            BKE_pbvh_node_mark_positions_update(*node);
          }
        }
    ...
    case Type::HideVert: {
      ...
      if (use_multires_undo(step_data, ss)) {
        ...
        for (bke::pbvh::Node *node : nodes) {
          if (indices_contain_true(
                  modified_grids,
                  bke::pbvh::node_grid_indices(static_cast<const bke::pbvh::GridsNode &>(*node))))
          {
                                   ...
                                   Node &unode)
{
  ...
  const Span<int> grid_indices = bke::pbvh::node_grid_indices(
      static_cast<const bke::pbvh::GridsNode &>(node));
...
static void store_coords(const Depsgraph &depsgraph, const Object &object, Node &unode)
{
  SculptSession &ss = *object.sculpt;
...
static void fill_node_data(const Depsgraph &depsgraph,
                           const Object &object,
                           const bke::pbvh::Node *node,
                           const Type type,
                           Node &unode)
{
  ...
  switch (type) {
    ...
    case Type::Position: {
      ...
      store_coords(depsgraph, object, unode);
      break;
    }
    case Type::HideVert: {
      if (ss.pbvh->type() == bke::pbvh::Type::Grids) {
               ...
               Type type)
{
  ...
  fill_node_data(depsgraph, object, node, type, *unode);
}
