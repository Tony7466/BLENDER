                                  ...
                                  MutableSpan<bool> modified_verts)
{
  ...
  threading::parallel_for(unodes.index_range(), 1, [&](const IndexRange range) {
    ...
    for (const int i : range) {
      ...
      translations_from_new_positions(unode.position.as_span().take_front(unode.unique_verts_num), verts, position_data.eval, translations);

      gather_data_mesh(position_data.eval, verts, unode.position.as_mutable_span().take_front(unode.unique_verts_num));
