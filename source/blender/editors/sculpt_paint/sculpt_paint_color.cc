  ...
  if (SCULPT_stroke_is_first_brush_step_of_symmetry_pass(*ss.cache)) {
    if (SCULPT_stroke_is_first_brush_step(*ss.cache)) {
      ss.cache->paint_brush.density_seed = float(
          BLI_hash_int_01(ss.cache->location_symmetry[0] * 1000));
    }
    return;
  }
                                ...
                                bke::GSpanAttributeWriter &color_attribute)
{
  ...
  if (brush.flag & BRUSH_ANCHORED) {
    ...
    brush_delta = ss.cache->location_symmetry - ss.cache->last_location_symmetry;
  }
  ...
  for (const int i : verts.index_range()) {
    ...
    switch (brush.smear_deform_type) {
      ...
      case BRUSH_SMEAR_DEFORM_PINCH:
        current_disp = ss.cache->location_symmetry - vert_positions[vert];
        break;
      case BRUSH_SMEAR_DEFORM_EXPAND:
        current_disp = vert_positions[vert] - ss.cache->location_symmetry;
        break;
    }
