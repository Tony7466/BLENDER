...
static int face_set_create_exec(bContext *C, wmOperator *op)
{
...
static void face_set_init_flood_fill(Object *ob, const FaceSetsFloodFillFn &test_fn)
{
  SculptSession *ss = ob->sculpt;
  Mesh *mesh = static_cast<Mesh *>(ob->data);
...
static int face_set_init_exec(bContext *C, wmOperator *op)
{
  ...
  switch (mode) {
    case InitMode::LooseParts: {
      ...
      face_set_init_flood_fill(ob,
                               [&](const int from_face, const int /*edge*/, const int to_face) {
...
static int face_set_change_visibility_exec(bContext *C, wmOperator *op)
{
  Object &object = *CTX_data_active_object(C);
  SculptSession *ss = object.sculpt;
  Depsgraph &depsgraph = *CTX_data_depsgraph_pointer(C);
...
static int face_set_change_visibility_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  Object *ob = CTX_data_active_object(C);
  SculptSession *ss = ob->sculpt;
  ...
  return face_set_change_visibility_exec(C, op);
}
...
void SCULPT_OT_face_set_change_visibility(wmOperatorType *ot)
{
  ...
  ot->exec = face_set_change_visibility_exec;
  ot->invoke = face_set_change_visibility_invoke;
  ot->poll = SCULPT_mode_poll;
...
static int face_set_randomize_colors_exec(bContext *C, wmOperator * /*op*/)
{
  Object *ob = CTX_data_active_object(C);
  SculptSession *ss = ob->sculpt;
...
void SCULPT_OT_face_sets_randomize_colors(wmOperatorType *ot)
{
  ...
  ot->exec = face_set_randomize_colors_exec;
  ot->poll = SCULPT_mode_poll;
...
static void face_set_grow_shrink(Object &object,
                                 const EditMode mode,
...
static void face_set_delete_geometry(Object *ob,
                                     const int active_face_set_id,
                                     const bool modify_hidden)
{
  Mesh &mesh = *static_cast<Mesh *>(ob->data);
...
static void face_set_edit_fair_face_set(Object *ob,
                                        const int active_face_set_id,
...
static bool face_set_edit_is_operation_valid(const Object &object,
                                             const EditMode mode,
                                             const bool modify_hidden)
{
...
static void face_set_edit_modify_geometry(bContext *C,
                                          Object *ob,
                                          ...
                                          wmOperator *op)
{
  ...
  switch (mode) {
    case EditMode::DeleteGeometry:
      face_set_delete_geometry(ob, active_face_set, modify_hidden);
      break;
    default:
      BLI_assert_unreachable();
  }
...
static void face_set_edit_modify_coordinates(
    bContext *C, Object *ob, const int active_face_set, const EditMode mode, wmOperator *op)
{
  ...
  switch (mode) {
    case EditMode::FairPositions:
      face_set_edit_fair_face_set(ob, active_face_set, MESH_FAIRING_DEPTH_POSITION, strength);
      break;
    case EditMode::FairTangency:
      face_set_edit_fair_face_set(ob, active_face_set, MESH_FAIRING_DEPTH_TANGENCY, strength);
      break;
    default:
      BLI_assert_unreachable();
  }
...
static bool face_set_edit_init(bContext *C, wmOperator *op)
{
  Object *ob = CTX_data_active_object(C);
  ...
  if (!face_set_edit_is_operation_valid(*ob, mode, modify_hidden)) {
    return false;
  }
...
static int face_set_edit_exec(bContext *C, wmOperator *op)
{
  if (!face_set_edit_init(C, op)) {
    return OPERATOR_CANCELLED;
  }
  ...
  switch (mode) {
    case EditMode::DeleteGeometry:
      face_set_edit_modify_geometry(C, ob, active_face_set, mode, modify_hidden, op);
      break;
    case EditMode::Grow:
    case EditMode::Shrink:
      face_set_grow_shrink(*ob, mode, active_face_set, modify_hidden, op);
      break;
    case EditMode::FairPositions:
    case EditMode::FairTangency:
      face_set_edit_modify_coordinates(C, ob, active_face_set, mode, op);
      break;
  }
...
static int face_set_edit_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  Depsgraph *depsgraph = CTX_data_depsgraph_pointer(C);
  Object *ob = CTX_data_active_object(C);
  SculptSession *ss = ob->sculpt;
  ...
  return face_set_edit_exec(C, op);
}
...
void SCULPT_OT_face_sets_edit(wmOperatorType *ot)
{
  ...
  ot->invoke = face_set_edit_invoke;
  ot->exec = face_set_edit_exec;
  ot->poll = SCULPT_mode_poll;
