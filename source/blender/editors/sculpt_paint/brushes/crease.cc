                       ...
                       const MutableSpan<float3> positions_orig)
{
  ...
  translations_from_position(positions_eval, verts, cache.location_symmetry, translations);

  if (brush.falloff_shape == PAINT_FALLOFF_SHAPE_TUBE) {
    project_translations(translations, cache.view_normal_symmetry);
  }
                       ...
                       LocalData &tls)
{
  ...
  const MutableSpan<float3> translations = tls.translations;
  translations_from_position(positions, cache.location_symmetry, translations);
