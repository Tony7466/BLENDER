...
class VertexSmearOperation : public GreasePencilStrokeOperationCommon {
 ...
 private:
  ...
  int2 get_offset_smear_direction(float2 dir);
                             ...
                             Fn &&fn)
  {
    ...
    points.foreach_index(grain_size, [&](const int64_t point) {
      ...
      const float2 translated = positions[point] - color_grid_.center - (half_size * cell_size_px);
...
void VertexSmearOperation::init_color_grid(const bContext &C, const float2 start_position)
{
  ...
  color_grid_.colors.fill(float3(0.0f));
    ...
    if (!point_selection.is_empty()) {
        ...
        for (const int x : IndexRange(color_grid_.size)) {
          ...
          point_selection.foreach_index([&](const int point_i) {
            ...
            if (math::distance_squared(cell_pos, view_pos) <= view_radius * view_radius) {
              color_grid_.colors[cell_i] += float3(color.r, color.g, color.b);
      ...
      for (const int cell_i : color_grid_.colors.index_range()) {
        if (points_per_cell[cell_i] > 0) {
          color_grid_.colors[cell_i] /= float(points_per_cell[cell_i]);
        }
      }
    }
    /* No update. */
...
/* Note: This is from the legacy grease pencil code. */
int2 VertexSmearOperation::get_offset_smear_direction(const float2 dir)
{
  /* Lower direction. */
  if (dir.y < 0.0f) {
    if ((dir.x >= -1.0f) && (dir.x < -0.8f)) {
      return {0, -1};
    }
    else if ((dir.x >= -0.8f) && (dir.x < -0.6f)) {
      return {-1, -1};
    }
    else if ((dir.x >= -0.6f) && (dir.x < 0.6f)) {
      return {-1, 0};
    }
    else if ((dir.x >= 0.6f) && (dir.x < 0.8f)) {
      return {-1, 1};
    }
    else if (dir.x >= 0.8f) {
      return {0, 1};
                                              ...
                                              const InputSample &extension_sample)
{
  ...
  const float2 mouse_vec = math::normalize(this->prev_mouse_position -
      ...
      this->foreach_point_on_grid(
          ...
          [&](const int point_i, const int2 grid_pos) {
            ...
            const int cell_i = math::clamp(
                smear_pos.y * color_grid_.size + smear_pos.x, 0, color_grid_.size - 1);

            const ColorGeometry4f mix_color = ColorGeometry4f(
                float4(color_grid_.colors[cell_i], 1.0f));
