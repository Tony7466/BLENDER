...
int ED_curve_updateAnimPaths(Main *bmain, Curve *cu)
{
  ...
  if (adt->action != nullptr) {
    ...
    DEG_id_tag_update(&adt->action->id, ID_RECALC_EVALUATED_COPY);
  }

  curve_rename_fcurves(cu, &adt->drivers);
  DEG_id_tag_update(&cu->id, ID_RECALC_EVALUATED_COPY);
  ...
  for (Object *obedit : objects) {
    ...
    if (changed) {
      DEG_id_tag_update(static_cast<ID *>(obedit->data),
                        ID_RECALC_EVALUATED_COPY | ID_RECALC_SELECT | ID_RECALC_GEOMETRY);
      WM_event_add_notifier(C, NC_GEOM | ND_SELECT, obedit->data);
      changed_multi = true;
    }
  }
                                   ...
                                   const SelectPick_Params *params)
{
        ...
        (((bezt ? (&bezt->f1)[hand] : bp->f1) & SELECT) != 0))
    {
      ...
      for (Object *ob_iter : objects) {
        ...
        DEG_id_tag_update(static_cast<ID *>(ob_iter->data),
                          ID_RECALC_SELECT | ID_RECALC_EVALUATED_COPY);
  ...
  if (found) {
    ...
    DEG_id_tag_update(static_cast<ID *>(obedit->data), ID_RECALC_SELECT | ID_RECALC_EVALUATED_COPY);
    WM_event_add_notifier(C, NC_GEOM | ND_SELECT, obedit->data);
