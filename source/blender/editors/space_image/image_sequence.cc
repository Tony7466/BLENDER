...
static void image_sequence_get_frame_ranges(wmOperator *op, ListBase *ranges)
{
  ...
  RNA_BEGIN (op->ptr, itemptr, "files") {
    ...
    if (do_frame_range) {
      LISTBASE_FOREACH (ImageFrameRange *, temp_range, ranges) {
        if (digits == temp_range->digits && STREQLEN(head, temp_range->head, FILE_MAX) &&
            STREQLEN(tail, temp_range->tail, FILE_MAX))
        {
          range = temp_range;
          break;
        }
    ...
    if (range) {
      ...
      range->digits = digits;
      STRNCPY(range->head, head);
      STRNCPY(range->tail, tail);
