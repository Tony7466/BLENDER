...
static void WIDGETGROUP_tool_generic_refresh(const bContext *C, wmGizmoGroup *gzgroup)
{
  ...
  /* skip, we don't draw anything anyway */
  {
    ...
    TransformCalcParams t_calc_params{};
    t_calc_params.use_only_center = true;
    t_calc_params.orientation_index = orientation + 1;
    const bool hide = ED_transform_calc_gizmo_stats(C, &t_calc_params, &tbounds, rv3d) == 0;
