...
static void draw_line_loop(const float coords[][3], int coords_len, const float color[4])
{
  ...
  blender::gpu::Batch *batch = GPU_batch_create_ex(GPU_PRIM_LINE_LOOP, vert, nullptr, GPU_BATCH_OWNS_VBO);
                            ...
                            const float color[4])
{
  ...
  GPU_blend(GPU_BLEND_ALPHA);
  blender::gpu::Batch *batch = GPU_batch_create_ex(GPU_PRIM_LINES, vert, nullptr, GPU_BATCH_OWNS_VBO);
  GPU_batch_program_set_builtin(batch, GPU_SHADER_3D_POLYLINE_UNIFORM_COLOR);
