                                              ...
                                              void *arg1)
{
  ...
  BMPartialUpdate_Params partial_update_params{};
  partial_update_params.do_tessellate = true;
  partial_update_params.do_normals = true;
  BMPartialUpdate *bmpinfo = BM_mesh_partial_create_from_verts_group_single(
      em->bm, &partial_update_params, verts_mask, verts_mask_count);
...
static TransformProperties *v3d_transform_props_ensure(View3D *v3d)
{
  if (v3d->runtime.properties_storage == nullptr) {
    v3d->runtime.properties_storage = MEM_cnew<TransformProperties>("TransformProperties");
  }
...
static void v3d_editvertex_buts(uiLayout *layout, View3D *v3d, Object *ob, float lim)
{
  ...
  if (block) { /* buttons */
    ...
    if (ob->type == OB_MESH) {
      ...
      BMEditMesh *em = me->edit_mesh;
      if (em != nullptr) {
        uiBlockInteraction_CallbackData interaction_callback_data{};
        interaction_callback_data.begin_fn = editmesh_partial_update_begin_fn;
        interaction_callback_data.end_fn = editmesh_partial_update_end_fn;
        interaction_callback_data.update_fn = editmesh_partial_update_update_fn;
        interaction_callback_data.arg1 = em;
        UI_block_interaction_set(block, &interaction_callback_data);
      }
    }
  }
  else { /* apply */
...
static void v3d_transform_butsR(uiLayout *layout, PointerRNA *ptr)
{
  ...
  uiItemR(colsub, ptr, "location", 0, nullptr, ICON_NONE);
  colsub = uiLayoutColumn(split, true);
  ...
  split = uiLayoutSplit(layout, 0.8f, false);
  colsub = uiLayoutColumn(split, true);
  uiItemR(colsub, ptr, "scale", 0, nullptr, ICON_NONE);
  colsub = uiLayoutColumn(split, true);
  uiLayoutSetEmboss(colsub, UI_EMBOSS_NONE_OR_STATUS);
  uiItemL(colsub, "", ICON_NONE);
  uiItemR(colsub,
...
void view3d_buttons_register(ARegionType *art)
{
  ...
  pt = MEM_cnew<PanelType>("spacetype view3d panel object");
  ...
  pt = MEM_cnew<PanelType>("spacetype view3d panel vgroup");
  ...
  mt = MEM_cnew<MenuType>("spacetype view3d menu collections");
