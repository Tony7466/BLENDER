  ...
  if (v3d) {
    ...
    if (obact && obact->type == OB_ARMATURE) {
      if (obact->mode & OB_MODE_POSE) {
        ...
        EditBone *ebone_act = (static_cast<bArmature *>(obact->data))->act_edbone;
        if (ebone_act) {
...
static int background_image_add_invoke(bContext *C, wmOperator *op, const wmEvent * /*event*/)
{
  ...
  Image *ima = reinterpret_cast<Image *>(WM_operator_drop_load_path(C, op, ID_IM));
...
static int background_image_remove_exec(bContext *C, wmOperator *op)
{
  ...
  if (bgpic_rem) {
    ...
    id_us_min(reinterpret_cast<ID *>(bgpic_rem->ima));
    id_us_min(reinterpret_cast<ID *>(bgpic_rem->clip));
...
static int drop_world_exec(bContext *C, wmOperator *op)
{
  ...
  World *world = reinterpret_cast<World *>(
      WM_operator_properties_id_lookup_from_name_or_session_uuid(bmain, op->ptr, ID_WO));
  if (world == nullptr) {
    return OPERATOR_CANCELLED;
  }

  id_us_min(reinterpret_cast<ID *>(scene->world));
  id_us_plus(&world->id);
  scene->world = world;
...
static int view3d_clipping_exec(bContext *C, wmOperator *op)
{
  ...
  rv3d->clipbb = MEM_cnew<BoundBox>("clipbb");
...
static int view3d_cursor3d_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  ...
  const eV3DCursorOrient orientation = eV3DCursorOrient(RNA_enum_get(op->ptr, "orientation"));
