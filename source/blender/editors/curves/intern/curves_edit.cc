...
void extrude_curve_points(bke::CurvesGeometry &curves, const IndexMask &mask)
{
  const OffsetIndices<int> points_by_curve = curves.points_by_curve();

  Array<bool> points_to_duplicate(curves.points_num());
  mask.to_bools(points_to_duplicate.as_mutable_span());
  const int num_points_to_add = 2 * mask.size();
  ...
  for (const int curve_i : curves.curves_range()) {
    ...
    for (const int point_index : points) {
      if (!curve_points_to_duplicate[point_index - points.first()]) {
        continue;
      }
      dst_to_src_point[curr_dst_point_start] = point_index;
      dst_to_src_point[curr_dst_point_start + 1] = point_index;
      dst_curve_counts.append(2);
      dst_to_src_curve.append(curve_i);
      curr_dst_point_start += 2;
    }
  }
  ...
  for (const StringRef selection_name : get_curves_selection_attribute_names(curves)) {
    bke::SpanAttributeWriter<bool> selection = attributes.lookup_or_add_for_write_span<bool>(
        selection_name, bke::AttrDomain::Point);
    for (int point_index = 0; point_index < num_points_to_add; ++point_index) {
      selection.span[old_points_num + point_index] = (point_index % 2 == 0);
    }
    selection.finish();
  }
}
