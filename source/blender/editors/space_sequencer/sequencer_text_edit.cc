...
static void cursor_move_character(TextVarsRuntime *text, int offset)
{
  ...
  /* Just to catch index errors here. */
  blender::seq::LineInfo llll = text->lines[text->cursor_line];
  blender::seq::CharInfo character = llll.characters[text->cursor_character];
}
