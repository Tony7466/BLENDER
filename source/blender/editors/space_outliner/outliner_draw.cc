                                                   ...
                                                   const bool value)
{
  ...
  if (base_or_object_prop) {
    ...
    LISTBASE_FOREACH (CollectionObject *, cob, &layer_collection->collection->gobject) {
      ...
      if (collection) {
        DEG_id_tag_update(&cob->ob->id, ID_RECALC_EVALUATED_COPY);
      }
    }
  }
...
static void namebutton_fn(bContext *C, void *tsep, char *oldname)
{
  ...
  if (ts && tselem) {
    ...
    if (tselem->type == TSE_SOME_ID) {
      ...
      DEG_id_tag_update(tselem->id, ID_RECALC_EVALUATED_COPY);
    }
    else {
      switch (tselem->type) {
        case TSE_DEFGROUP: {
        ...
        case TSE_GREASE_PENCIL_NODE: {
          ...
          DEG_id_tag_update(&grease_pencil.id, ID_RECALC_EVALUATED_COPY);
          WM_event_add_notifier(C, NC_ID | NA_RENAME, nullptr);
          break;
        }
        case TSE_R_LAYER: {
