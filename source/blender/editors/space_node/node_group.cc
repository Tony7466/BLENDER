...
static void node_group_ungroup(Main *bmain, bNodeTree *ntree, bNode *gnode)
{
  ...
  for (bNode *node : nodes_delayed_free) {
    blender::bke::nodeRemoveNode(bmain, ntree, node, false);
  }
  ...
  blender::bke::nodeRemoveNode(bmain, ntree, gnode, true);
}
    ...
    Main &bmain, bNodeTree &ntree, bNodeTree &ngroup, const float2 &offset, const bool make_copy)
{
  ...
  for (bNode *node : nodes_to_move) {
    ...
    if (make_copy) {
      ...
      const int32_t old_identifier = node->identifier;
      blender::bke::nodeUniqueID(&ntree, newnode);
      blender::bke::nodeUniqueName(&ntree, newnode);
    ...
    if (newnode->parent && !(newnode->parent->flag & NODE_SELECT)) {
      blender::bke::nodeDetachNode(&ngroup, newnode);
    }
  ...
  LISTBASE_FOREACH_MUTABLE (bNodeLink *, link, &ngroup.links) {
    ...
    if (make_copy) {
      ...
      if (fromselect && toselect) {
        ...
        blender::bke::nodeRemLink(&ngroup, link);
      }
    }
  }
                                          ...
                                          ReportList &reports)
{
  ...
  bNodeTree *ngroup = blender::bke::ntreeAddTree(nullptr, "Pseudo Node Group", ntree_idname);
  BLI_SCOPED_DEFER([&]() {
                                            ...
                                            const VectorSet<bNode *> &nodes_to_move)
{
  ...
  bNode *output_node = [&]() {
    ...
    bNode *output_node = blender::bke::nodeAddStaticNode(&C, &group, NODE_GROUP_OUTPUT);
  ...
  bNode *input_node = blender::bke::nodeAddStaticNode(&C, &group, NODE_GROUP_INPUT);
  ...
  for (bNode *node : nodes_to_move) {
    ...
    blender::bke::nodeUniqueID(&group, node);
    blender::bke::nodeUniqueName(&group, node);
  ...
  for (const NewInternalLinkInfo &info : new_internal_links) {
    ...
    if (info.socket->in_out == SOCK_IN) {
      ...
      blender::bke::nodeAddLink(&group, input_node, input_socket, info.node, info.socket);
    }
    else {
      bNodeSocket *output_socket = node_group_output_find_socket(output_node,
                                                                 io_identifier.c_str());
      blender::bke::nodeAddLink(&group, info.node, info.socket, output_node, output_socket);
    }
  }
  ...
  for (const auto item : input_links.items()) {
    ...
    bNodeSocket *group_node_socket = node_group_find_input_socket(gnode,
                                                                  interface_identifier.c_str());
    blender::bke::nodeAddLink(&ntree, item.value.from_node, item.key, gnode, group_node_socket);
  }
                                         ...
                                         const char *ntreetype)
{
  ...
  /* New node-tree. */
  bNodeTree *ngroup = blender::bke::ntreeAddTree(bmain, "NodeGroup", ntreetype);

  /* make group node */
  bNode *gnode = blender::bke::nodeAddNode(&C, &ntree, ntype);
  gnode->id = (ID *)ngroup;
...
static int node_group_make_exec(bContext *C, wmOperator *op)
{
  ...
  if (gnode) {
    ...
    blender::bke::nodeSetActive(&ntree, gnode);
    if (ngroup) {
      ED_node_tree_push(&snode, ngroup, gnode);
    }
  }
...
static int node_group_insert_exec(bContext *C, wmOperator *op)
{
  ...
  for (const bNode *group : nodes_to_group) {
    ...
    if (blender::bke::ntreeContainsTree(reinterpret_cast<bNodeTree *>(group->id), ngroup)) {
      BKE_reportf(
