                      ...
                      bNodeLink &link_to_pick)
{
  ...
  blender::bke::nodeRemLink(snode.edittree, &link_to_pick);
                                    ...
                                    int replace)
{
  ...
  if (replace) {
    blender::bke::nodeRemSocketLinks(ntree, sock_to);
  }

  blender::bke::nodeAddLink(ntree, node_fr, sock_fr, node_to, sock_to);
  return true;
}
  ...
  LISTBASE_FOREACH_MUTABLE (bNodeLink *, link, &btree.links) {
    if (link->tonode == &viewer_node) {
      if (link->tosock->flag & SOCK_UNAVAIL) {
        blender::bke::nodeRemLink(&btree, link);
      }
    }
  }
}
                       ...
                       bNodeSocket &bsocket_to_view)
{
  ...
  if (viewer_link == nullptr) {
    viewer_link = blender::bke::nodeAddLink(
...
{
  SpaceNode &snode = *CTX_wm_space_node(C);
  bNode *node = blender::bke::nodeGetActive(snode.edittree);
  ...
  while (socket != socket_to_match) {
    if (!socket->is_hidden() && socket->is_available()) {
      ...
      if (sockets_are_compatible) {
        ...
        const bool socket_has_capacity = link_count < blender::bke::nodeSocketLinkLimit(socket);
...
static void displace_links(bNodeTree *ntree, const bNode *node, bNodeLink *inserted_link)
{
  ...
  if (linked_socket->is_input()) {
    ...
    if (!replacement_socket) {
      blender::bke::nodeRemLink(ntree, displaced_link);
      return;
    }
...
static void node_remove_existing_links_if_needed(bNodeLinkDrag &nldrag, bNodeTree &ntree)
{
  ...
  const int link_limit = blender::bke::nodeSocketLinkLimit(&linked_socket);
                                                     ...
                                                     const bool detach)
{
  if (bNodeSocket *sock = node_find_indicated_socket(snode, region, cursor, SOCK_OUT)) {
    ...
    nldrag->start_link_count = blender::bke::nodeCountSocketLinks(snode.edittree, sock);
    int link_limit = blender::bke::nodeSocketLinkLimit(sock);
    if (nldrag->start_link_count > 0 && (nldrag->start_link_count >= link_limit || detach)) {
      ...
      LISTBASE_FOREACH_MUTABLE (bNodeLink *, link, &snode.edittree->links) {
        if (link->fromsock == sock) {
          ...
          blender::bke::nodeRemLink(snode.edittree, link);
        }
      }
    }
    else {
...
static int node_parent_set_exec(bContext *C, wmOperator * /*op*/)
{
  SpaceNode &snode = *CTX_wm_space_node(C);
  bNodeTree &ntree = *snode.edittree;
  bNode *frame = blender::bke::nodeGetActive(&ntree);
  ...
  for (bNode *node : ntree.all_nodes()) {
    ...
    if (node->flag & NODE_SELECT) {
      ...
      blender::bke::nodeAttachNode(&ntree, node, frame);
    }
  }
...
static int node_join_exec(bContext *C, wmOperator * /*op*/)
{
  ...
  bNode *frame_node = blender::bke::nodeAddStaticNode(C, &ntree, NODE_FRAME);
  blender::bke::nodeSetActive(&ntree, frame_node);
...
static int node_attach_invoke(bContext *C, wmOperator * /*op*/, const wmEvent *event)
{
  ...
  for (bNode *node : tree_draw_order_calc_nodes_reversed(*snode.edittree)) {
    ...
    if (node->is_frame() && blender::bke::nodeIsParentAndChild(node, frame)) {
      continue;
    }
    ...
    const bool share_parent = blender::bke::nodeIsParentAndChild(node->parent, frame);
    if (!share_parent) {
      continue;
    }
...
void node_insert_on_link_flags(Main &bmain, SpaceNode &snode)
{
  ...
  if (best_output != nullptr) {
    ...
    blender::bke::nodeRemLink(&ntree, old_link);
  }

  if (best_input != nullptr) {
    /* Add a new link that connects the node on the left to the newly inserted node. */
    blender::bke::nodeAddLink(&ntree, from_node, from_socket, node_to_insert, best_input);
  }
...
static void node_parent_offset_apply(NodeInsertOfsData *data, bNode *parent, const float offset_x)
{
  ...
  for (bNode *node : data->ntree->all_nodes()) {
    if (blender::bke::nodeIsParentAndChild(parent, node)) {
      /* NODE_TEST is used to flag nodes that shouldn't be offset (again) */
      node->flag |= NODE_TEST;
    }
  }
                                             ...
                                             const bool reversed)
{
  ...
  if (data->insert_parent) {
    ...
    if (!blender::bke::nodeIsParentAndChild(data->insert_parent, ofs_node)) {
  ...
  if (dist < min_margin) {
    ...
    if (needs_alignment) {
      ...
      if (!offs_node->parent || offs_node->parent == insert.parent ||
          blender::bke::nodeIsParentAndChild(offs_node->parent, &insert))
      {
