    ...
    const SpaceNode &snode)
{
  ...
  for (const int i : treepath.index_range().drop_back(1)) {
    ...
    const bNode *node = blender::bke::nodeFindNodebyName(tree, treepath[i + 1]->node_name);
    if (node == nullptr) {
                                        ...
                                        const char *route_name)
{
  ...
  for (int i = treepath.size() - 1; i > 0; --i) {
    ...
    if (output_node == nullptr) {
      output_node = blender::bke::nodeAddStaticNode(nullptr, nested_nt, NODE_GROUP_OUTPUT);
    ...
    blender::bke::nodeAddLink(nested_nt, nested_node_iter, nested_socket_iter, output_node, out_socket);
    BKE_ntree_update_main_tree(G.pr_main, nested_nt, nullptr);
    ...
    nested_node_iter = blender::bke::nodeFindNodebyName(path_prev->nodetree, path->node_name);
  ...
  blender::bke::nodeAddLink(treepath.first()->nodetree,
              nested_node_iter,
              nested_socket_iter,
              &final_node,
              &final_socket);
                                           ...
                                           bNode &output_node)
{
  ...
  out_surface_socket = blender::bke::nodeFindSocket(&output_node, SOCK_IN, "Surface");
  if (out_surface_socket->link) {
    /* Make sure no node is already wired to the output before wiring. */
    blender::bke::nodeRemLink(main_nt, out_surface_socket->link);
  }
                                  ...
                                  const Span<NodeSocketPair> &nodesocket_span)
{
  ...
  for (NodeSocketPair nodesocket : nodesocket_span) {
    ...
    bNode *aov_node = blender::bke::nodeAddStaticNode(nullptr, main_nt, SH_NODE_OUTPUT_AOV);
    STRNCPY(reinterpret_cast<NodeShaderOutputAOV *>(aov_node->storage)->name,
            nodesocket.first->name);
    if (socket_preview == nullptr) {
      continue;
    }
    bNodeSocket *aov_socket = blender::bke::nodeFindSocket(aov_node, SOCK_IN, "Color");
    if (socket_preview->in_out == SOCK_IN) {
...
static bool prepare_viewlayer_update(void *pvl_data, ViewLayer *vl, Depsgraph *depsgraph)
{
  ...
  if (job_data->mat_displacement_copy.first != nullptr && displacement_socket->link == nullptr) {
    blender::bke::nodeAddLink(job_data->treepath_copy.first()->nodetree,
...
static void shader_preview_startjob(void *customdata, wmJobWorkerStatus *worker_status)
{
  ...
  for (bNode *node_iter : job_data->mat_copy->nodetree->all_nodes()) {
    if (node_iter->flag & NODE_DO_OUTPUT) {
      ...
      bNodeSocket *disp_socket = blender::bke::nodeFindSocket(node_iter, SOCK_IN, "Displacement");
  ...
  job_data->mat_output_copy = blender::bke::nodeAddStaticNode(
                                     ...
                                     ListBase &treepath)
{
       ...
       original_path = original_path->next)
  {
    ...
    bNode *parent = blender::bke::nodeFindNodebyName(job_data->treepath_copy.last()->nodetree,
