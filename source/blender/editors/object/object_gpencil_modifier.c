...
static int gpencil_surfacedeform_bind_exec(bContext *C, wmOperator *op)
{
  ...
  PointerRNA ob_ptr;
  RNA_pointer_create(&ob->id, &RNA_GpencilModifier, smd, &ob_ptr);
  ...
  if (smd->target) {
    smd->flags |= GP_MOD_SDEF_DO_BIND;
  }
  
  if (unbind_mode)
  {
    smd->bind_modes |= GP_MOD_SDEF_UNBIND_MODE;
  }
  else
  {
    smd->bind_modes &= ~GP_MOD_SDEF_UNBIND_MODE;
  ...
  {
    smd->bind_modes |= GP_MOD_SDEF_BIND_CURRENT_FRAME;
    smd->bind_modes &= ~GP_MOD_SDEF_BIND_ALL_FRAMES;
  }
  else
  {
    smd->bind_modes &= ~GP_MOD_SDEF_BIND_CURRENT_FRAME;
  ...
  {
    smd->bind_modes |= GP_MOD_SDEF_BIND_CURRENT_LAYER;
    smd->bind_modes &= ~GP_MOD_SDEF_BIND_ALL_LAYERS;
  }
  else
  {
    smd->bind_modes &= ~GP_MOD_SDEF_BIND_CURRENT_LAYER;
