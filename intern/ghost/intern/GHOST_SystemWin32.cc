 ...
 * Windows API #setCursorPos sets the cursor at specific location, but following cursor events
 * can be outdated or can have new movement since the position was set, #clip_cursor_at_point
 * helps to reduce the iterations needed to regonize when this position was successfully updated.
 */
 ...
 * edge.
 */
static int32_t wrap_axis(const int32_t position, const int32_t min, const int32_t max)
{
  if (position == min) {
    return max - 1;
  }
...
static BOOL clip_cursor_at_bounds(const GHOST_Rect &bounds,
                                  ...
                                  int32_t &y)
{
  ...
  if (warp_y) {
    /* The y-axis clipping is is defined by `top <= y < bottom`. */
                                                         ...
                                                         const int32_t screen_co[2])
{
  ...
  if (bounds_axis != GHOST_kAxisNone) {
    ...
    clip_cursor_at_bounds(bounds, bounds_axis, 1, x_new, y_new);
    ...
    if (x_new != x_screen || y_new != y_screen) {
      clip_cursor_at_point(x_new, y_new);
      is_warping = true;
