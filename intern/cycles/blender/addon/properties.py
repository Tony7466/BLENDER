...
def enum_optix_denoiser(self, context):
    if not context or bool(context.preferences.addons[__package__].preferences.get_devices_for_type('OPTIX')):
        return [('OPTIX', "OptiX", tip_("Use the OptiX AI denoiser with GPU acceleration, only available on NVIDIA GPUs when configured in the system tab in the user preferences"), 2)]
    return []
...
def enum_preview_denoiser(self, context):
    ...
    if len(optix_items) or len(oidn_items):
        items = [
            ('AUTO',
             "Automatic",
             tip_("Use GPU accelerated denoising if supported, for the best performance. "
                  "Prefer OpenImageDenoise over OptiX"),
             0)]
    else:
        items = [('AUTO', "None", tip_("Blender was compiled without a viewport denoiser"), 0)]
