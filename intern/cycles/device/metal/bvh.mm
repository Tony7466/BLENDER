...
#  if defined(MAC_OS_VERSION_14_0)
  if (@available(macos 14.0, *)) {
    ...
    if (motion_blur) {
        ...
        for(int c =0; c < numCurves; ++c) {
          ...
          for(int s =0; s < segCount;++s)
          {
              if (step == 0) {
                ...
                assert(idxData.size() <= numSegments);
              }
      ...
      geomDescCrv.curveType = (hair->curve_shape == CURVE_RIBBON) ? MTLCurveTypeFlat : MTLCurveTypeRound;
                          ...
                          bool refit)
{
  if (@available(macos 12.0, *)) {
    ...
    [all_blas enumerateObjectsUsingBlock:
        ^(id<MTLAccelerationStructure> blas, NSUInteger idx, BOOL *stop) {
