...
class HIPRTDevice : public HIPDevice {
 ...
 protected:
  ...
  // Originally, visibility was only passed to HIP RT but after a bug report it was noted it was required for custom primitives (i.e., motion triangles)
  // This buffer, however, has visibility per object not per primitive so the same buffer as the one that is passed to HIP RT can be used
