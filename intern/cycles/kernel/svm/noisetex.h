                              ...
                              bool hard)
{
  switch ((NodeNoiseType)type) {
    ...
    case NODE_NOISE_FBM: {
      return noise_fbm(p, detail, roughness, lacunarity, normalize, hard);
    }
                                 ...
                                 ccl_private float3 *color)
{
  ...
  *value = noise_select(p, detail, roughness, lacunarity, offset, gain, type, normalize, hard);
  if (color_is_needed) {
