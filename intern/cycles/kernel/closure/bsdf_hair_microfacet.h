...
/* Albedo correction. Treat as glass. `rough` has alraedy applied square root. */
ccl_device_forceinline float bsdf_microfacet_hair_E(KernelGlobals kg,
                                                    float mu,
                                                    float rough,
                                                    float ior)
{
  bool inv_table = (ior < 1.0f);
  int ofs = inv_table ? kernel_data.tables.ggx_glass_inv_E : kernel_data.tables.ggx_glass_E;
    ...
    KernelGlobals kg, const float roughness, const float3 wi, const float3 wm, const float2 rand)
{
  ...
  const float3 wh_wm = (m_type == MicrofacetType::GGX) ?
                           microfacet_ggx_sample_vndf(wi_wm, roughness, roughness, rand) :
                           microfacet_beckmann_sample_vndf(wi_wm, roughness, roughness, rand);
  ...
  for (size_t i = 0; i <= intervals; i++) {
    ...
    if (microfacet_visible(wi, wo, make_float3(wm.x, 0.0f, wm.z), wh)) {
      ...
      integral += weight * bsdf_D<m_type>(roughness2, dot(wm, wh)) * G *
                  ...
                  bsdf_microfacet_hair_E(kg, cos_mi, sqrtf(roughness), bsdf->eta);
    }
  }
                                                       ...
                                                       uint rng_quadrature)
{
  ...
  for (size_t i = 0; i <= intervals; i++) {
    ...
    const float scale1 = 1.0f / bsdf_microfacet_hair_E(kg, cos_mi1, sqrt_roughness, eta);
    ...
    const float scale2 = 1.0f / bsdf_microfacet_hair_E(kg, cos_mi2, sqrt_roughness, inv_eta);
    ...
    if (bsdf->extra->TRT > 0.0f) {
      ...
      const float T3 = (1.0f - fresnel_dielectric_cos(cos_hi3, inv_eta)) /
                       bsdf_microfacet_hair_E(kg, cos_mi3, sqrt_roughness, inv_eta);
                                           ...
                                           ccl_private float *eta)
{
  ...
  *eval = zero_spectrum();
  ...
  const float scale1 = 1.0f / bsdf_microfacet_hair_E(kg, cos_mi1, sqrt_roughness, bsdf->eta);
