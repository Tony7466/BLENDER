                                                  ...
                                                  float3 *layer_albedo)
{
  ...
  ccl_private HairBsdf *bsdf = (ccl_private HairBsdf *)bsdf_alloc(sd, sizeof(HairBsdf), weight);
  if (!bsdf) {
    return;
  }
...
ccl_device void osl_closure_absorption_setup(KernelGlobals kg,
                                             ...
                                             float3 *layer_albedo)
{
  volume_extinction_setup(sd, weight * object_volume_density(kg, sd->object));
}
