...
template<typename AT> inline bool is_symmetric(const Eigen::SparseMatrix<AT> &A);
// Inputs:
//   epsilon threshold on L1 difference between A and A'
template<typename AT, typename epsilonT>
inline bool is_symmetric(const Eigen::SparseMatrix<AT> &A, const epsilonT epsilon);
template<typename DerivedA> inline bool is_symmetric(const Eigen::PlainObjectBase<DerivedA> &A);
}  // namespace igl
