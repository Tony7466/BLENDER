                     ...
                     bool uniform)
{
  ...
  if (!uniform) {
    ...
    //
    // Use normals of the uniform tet (V =
    // h*[0,0,0;1,0,0;0.5,sqrt(3)/2.,0;0.5,sqrt(3)/6.,sqrt(2)/sqrt(3)])
    //         0         0    1.0000
    ...
    for (int i = 0; i < m; i++) {
      N.row(0 * m + i) << 0, 0, 1;
      double a = sqrt(2) *
                 std::cbrt(3 * vol(i));  // area of a face in a uniform tet with volume = vol(i)
