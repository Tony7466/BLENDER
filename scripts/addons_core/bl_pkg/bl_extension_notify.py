...
def splash_draw_status_fn(layout, context):
    ...
    if not bpy.app.online_access:
        if bpy.app.online_access_override:
            ...
            layout.label(text="Running in Offline Mode", icon='INTERNET')
            return
    ...
    if notify.sync_info is None:
        syncing = True
    else:
        status_data, update_count, _ = notify.sync_info
    ...
    props = row.operator("bl_pkg.extensions_show", text=text, icon='PREFERENCES')
    props.for_update = update_count > 0
    if notify.do_online_sync and syncing:
